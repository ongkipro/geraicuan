...
const geometry=await s.evaluate(`(()=>{const h=document.querySelector('[data-slot=cms-header]'),t=h.querySelector('time'),r=h.getBoundingClientRect();return {height:r.height,overflow:document.documentElement.scrollWidth-innerWidth,time:t.textContent,date:t.dateTime}})()`);assert(geometry.overflow<=0);assert(geometry.time.includes('WIB')&&!/\d{2}[.:]\d{2}[.:]\d{2}/.test(geometry.time));assert(await s.evaluate(`(()=>{const b=document.querySelector('button[aria-label="Cari halaman"]'),r=b.getBoundingClientRect();return [...b.children].filter(e=>e.getBoundingClientRect().width>0).every(e=>{const c=e.getBoundingClientRect();return c.left>=r.left&&c.right<=r.right})})()`),'Search trigger child bounds');
// V-30 (T-202): minutes only, no per-second tick; below md the clock is not shown.
assert.equal(await s.evaluate(`getComputedStyle(document.querySelector('[data-slot=cms-header] time').parentElement).display==='none'`),width<768);
