...
try {
 ...
 for(const width of [1440,390]){
  ...
  assert.equal(form.locked,false,'dev tenant prefix is still choosable');assert.equal(form.value,'LDT');assert.equal(form.maxLength,5);assert(form.preview);assert(form.buttonHeight>=44||width>=768&&form.buttonHeight>=36,JSON.stringify(form));
  ...
  await s.evaluate(`(()=>{const i=document.getElementById('shipment-prefix');Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set.call(i,'LDT');i.dispatchEvent(new Event('input',{bubbles:true}))})()`);
  ...
  assert.match(dialog.title,/Kunci awalan LDT-\?/);assert(dialog.warns);assert.deepEqual([dialog.confirmName,dialog.confirmValue,dialog.confirmForm],['confirmation','locked','shipment-prefix-form']);assert(dialog.focusInside);
 ...
 await s.goto(origin+'/app/label/10024');await wait(`!!document.querySelector('.label-footer p')`);await pause(300);
