...
// T-188: below md the role lists are cards; the pinned-name table starts at
// md, so its sticky column is measured at 768 where the table still overflows.
await s.send("Emulation.setDeviceMetricsOverride",{width:768,height:900,deviceScaleFactor:1,mobile:false});
...
// T-188: both role lists carry the same pinned-column table.
for (const route of ["/app/kontak/pengirim", "/app/kontak/penerima"]) {
  await s.goto(`${ORIGIN}${route}`); await new Promise(r=>setTimeout(r,700));
  const d=JSON.parse(await s.evaluate(PROBE));
  console.log(`768px ${route} ok=`+d.stickyOk, JSON.stringify(d.stickyIssues));
}
