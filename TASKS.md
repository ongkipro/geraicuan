...
> Complete the accepted GeraiCUAN Mengantar location-authority and outlet-settings goal from the repository's current state. First inspect `delivery-ledger status` and this queue: resume a valid active atomic run, otherwise execute T-52 and then T-53 in dependency order. T-52 owns the remaining server-side destination-area search and one provider-ID/label binding through contacts, drafts, estimates, and order payload construction; T-53 owns the responsive many-outlet settings composition after that contract is complete. Treat repository requirements, disk state, and fresh executable evidence as authoritative. Use `mengantar-api` for provider-contract work, `full-stack-development` for cross-layer implementation, `shadcn-ui` for admin components, and `designer` or `vision` before and after browser-visible edits. Obtain the required independent review, pass the delivery boundary, update repository evidence, and continue until both tasks are genuinely complete. Never create a provider order, mutate production, read or print secrets, commit, push, deploy, or release without explicit approval.
...
4. Select or resume exactly one unblocked atomic task. T-52 must close before T-53 begins; their backend/data and responsive-UI boundaries must not be merged into one run.
...
T-52 closes only after the accepted provider search contract is implemented server-side and the same validated destination ID/label pair survives contact selection, draft persistence, estimation, and provider-order payload construction without a real provider mutation. T-53 then closes only after one responsive outlet context works for zero, one, and many outlets and a Tenant Admin can discover, change, reload, and recover the settings workflow at every required viewport. A visual defect found during T-53 stays in T-53; a provider-authority, persistence, tenant-isolation, or secret-boundary defect reopens T-52.
...
Execution order is strict: complete T-52's provider-authoritative destination
contract and downstream data binding first, then complete T-53's responsive
many-outlet experience. Existing pickup-selector work is accepted progress, not
permission to mark either task complete.
...
- [ ] **T-52 — Complete Mengantar destination location authority**
  - Primary requirement: PR-28
  - Constraints: PR-3, PR-5, NFR-1, NFR-2, NFR-3, TD-2, TD-16, DATA-7, UX-10
  - Dependencies: T-50
  - Scope: Keep the accepted pickup implementation and complete the official general area-search contract as a bounded server-only lookup with strict response validation, timeout/size/concurrency limits, sanitized errors, and no credential-bearing URL logging. Bind one provider destination ID and readable hierarchy through contact addresses, immutable shipment-party snapshots, drafts, estimates, and provider-order payload construction. Do not add a cache or canonical kecamatan table without measured evidence, create an order, or treat a syntactically valid opaque ID as provider authority.
  - Done when: Sanitized contract fixtures and disposable-database tests prove valid hierarchy mapping, query normalization and bounds, no-result, malformed, timeout, rate/concurrency rejection, and provider-unavailable behavior; current-account pickup authority and destination ID/label bindings survive tenant-scoped contact/draft/estimate/order-payload flows; stale account/version writes fail closed; no secret/PII leaks; and any non-mutating sandbox probe is separately approved and recorded rather than implied by fixture success.
  - Progress evidence (2026-09-01): the official Mengantar docs now establish account pickup `GET /api/public/{API_KEY}/address`, pickup `_id`, and origin-area `PICKUP_AUTOFILL`. The outlet-settings slice implements bounded server-only account lookup, strict safe DTO mapping, platform shared-account filtering, pickup/area label persistence, and a connection-version check that rejects credential-change races. Migration 0026 preserves legacy ID-only rows. General destination search and ID/label binding through contacts, drafts, estimates, and orders remain before T-52 can close.
...
- [ ] **T-53 — Replace opaque location IDs with the complete outlet settings experience**
  ...
  - Progress evidence (2026-09-01): manual opaque pickup/origin fields are replaced by a shadcn `Command`/`Popover` pickup selector with provider labels, derived read-only origin, loading, account-empty, query-empty, retry, legacy migration, accessible selected-state, and focus-recovery behavior. The many-outlet desktop list-detail and mobile single-outlet selector composition remains before T-53 can close.
