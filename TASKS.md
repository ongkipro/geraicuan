...
- [ ] **T-229 — Informasi label editor (PR-86).** Tenant setting per size (additive migration), `Pengaturan → Informasi label` with toggles + live preview reusing the label sheet, label sheet reads the setting. Integration + browser evidence.
...
- [ ] **T-233 — Gerai contact on the nota is editable by the owner.** The gerai WhatsApp printed on label and invoice comes from registration (`tenants.contact_whatsapp`) and no screen or action changes it; add it (and show it) on Pengaturan → Profil gerai with a Tenant Admin action and RLS-safe update. Found in the 2026-09-26 page audit.
