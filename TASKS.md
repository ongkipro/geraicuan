...
- [x] **T-234 — No native selects; pickup window 08.00–17.00; member role cards (owner 2026-09-26).** Done 2026-09-26: (1) every native `<select>` replaced by the shadcn Select — Buat kiriman pickup date/slot (calendar/clock icon inside the trigger; values still post through the existing hidden `pickupDate`/`pickupSlot` inputs, the Selects carry no `name`), Histori status facet (`defaultValue` + `key`, navigation on change, URL stays the state), status-pull outlet (Select rendered outside the `<form action>` because React resets the form after the action and a Radix Select listens to `reset`; the value posts through a hidden `outletId`); `grep -rn "<select" src` → 0. (2) Slots 08:00–16:00 starts (08.00–17.00 WIB), ≥ 90 min rule unchanged; migration 0061 widens `shipment_drafts_pickup_slot_valid` to the superset `^(0[89]|1[0-7]):00$` (17:00 legacy only); D-25, DATA-13, PR-70, spec 17 note that Mengantar documents 09:00–18:00 (unverified) and nothing is sent until T-153. (3) Undang anggota and Kelola akses role = two radio option cards (`OptionCard` moved to `src/components/app/option-card.tsx`, `RoleChoice` keyed on the action result), same `role` field/values. Evidence: related tests 10 files / 124 passed on the iso DB (new `tests/native-select-replacement-t234.integration.test.ts`: rendered form posts exactly one `pickupDate`/`pickupSlot` accepted by `validateShipmentDraft`; DB test stores 08:00, keeps 17:00, refuses 07:00/18:00); `verify-migration-upgrade` through 0061 on a temporary DB (created/dropped); 0061 applied to the iso and dev DBs; tsc 0; lint 0. Browser (dev app, 1440/390, nothing submitted): slot list today at 11.09 WIB = 13.00–16.00 only; tomorrow 08.00–16.00, 08.00 chosen → `FormData` `pickupDate=2026-09-27`, `pickupSlot=08:00`; triggers 40 px / 44 px; Histori facet → `?…&status=DELIVERED` with period kept; invite cards and Kelola akses dialog; screenshots in the session scratchpad `selects/`.
- [x] **T-235 — Laporan pengiriman analytics (owner 2026-09-26: "laporan pengiriman mungkin kurang lengkap, tambahkan analitik, diagram jika perlu, mapping jika memungkinkan").** Done 2026-09-26: `loadShipmentReportAnalytics` in `src/db/shipment-report-repository.ts` (same `reportJoins`/`reportFilter` cohort, tenant column predicate, Tenant Admin only; SQL aggregates for KPIs, WIB buckets, outlet × area label) + per-courier `deliveredCount`/`returnedCount`; one area parser `parseAreaRegion` (`label-format.ts`) and `groupRegions`/`groupRoutes` (`src/lib/shipment-report-analytics.ts`); page sections (`analytics-sections.tsx`, `report-trend-chart.tsx`, `region-bar-chart.tsx`, shadcn-style `ui/tabs.tsx` on the installed `radix-ui`): Ringkasan 6 KPI cards, Tren harian (COD solid / Non-COD dashed, Tabs to Nilai COD per hari, data-table disclosure), Distribusi status (replaces Total per status), Total per kurir + % terkirim / % retur, Wilayah tujuan (Per provinsi / Per kota, top 10 bar + table + "Tampilkan semua", "Wilayah tidak dikenal"), Rute teratas (outlet → kota, top 5). Metrics RPT-SHP-DELIVERED/-RETURNED/-RETURN-RATE/-IN-PROGRESS/-COD-VALUE-TOTAL/-TREND-*/-REGION-*/-ROUTE-COUNT/-COURIER-*-RATE registered in spec 19 with the Dasbor alignment stated; spec 17/18 rows updated. No map (not in scope). Evidence (iso DB 55462, never 55461): `shipment-report` + `report-pages-render` 2 files / 32 passed (5 new DB cases incl. reconciliation with Histori count, casing-blind city, unknown last, tenant predicate on analytics SQL; 5 new render/logic cases); full suite 115 files / 1,342 passed / 4 failed — the 4 are `platform-public-render` T-225 visual panel and three `tenant-registration` security cases, outside this change (auth shell/registration work in progress in the worktree); tsc 0; lint 0. Browser (dev app, isolated context, 1440/1024/390): no document overflow, all tables fit their card, 3 charts render, tabs switch by ArrowRight with `aria-selected`, no text under 13 px; Dasbor 30 hari shows the same 134 / 45 / 18 / 65. Fix during check: region axis names cut at 22 characters (width from the cut name); Total per kurir switches to records below xl (7 columns overflowed 8 px at 1024). Gaps: rates in region/route/courier tables do not carry the M-0 low-volume caption (n < 10); `ui/tabs.tsx` hand-written (registry not fetched).
- [x] **T-236 — Operator-role pass + Laporan low-volume guard (spec 19 M-0).** Done 2026-09-26. (1) Operator pass on the dev app (`operator@geraicuan.com`, isolated context `t236-operator`, 1440 and 390): every menu item (Dasbor, Buat kiriman, Histori, Retur, Cetak resi, Pengirim, Penerima, Cek resi, Cek tarif; the phone sheet and Cmd-K list the same nine), detail GC-10178/10176/10171/10159/10109, label, label `?invoice=1`, invoice, batch print, contact list/archived/detail/new, Cek resi, Cek tarif. No Perbarui status, Laporan, Pengaturan/Anggota, cost columns, stale filters, archive zone, prefix/WhatsApp/label settings; 125 in-page links fetched as the Operator, none redirects or 403s; document overflow 0 on 15 pages × 2 widths; allowed actions present (Buat kiriman + Simpan & cek tarif, Cetak resi + invoice, Terbitkan invoice, Pengirim/Penerima baru, contact edit/Tambah alamat, Cek resi/tarif). No shipment submitted, no invoice issued, no data changed. Fixed: Dasbor "Lihat panduan admin" → "Lihat panduan", and an Operator's SUBMISSION_UNKNOWN row no longer promises "Periksa rekonsiliasi" (`dashboard-logic.ts`, test in `dashboard-v3`); Histori stale-filter notice and the member note say "Pemilik gerai"/"pemilik gerai lain"; `/app/anggota` and `/app/laporan/*` flashed their admin skeleton (Anggota & akses + settings sub-menu) before the redirect → guard-only `anggota/layout.tsx` and `laporan/layout.tsx` above `loading.tsx`, so all eight admin URLs now show only the Dasbor skeleton then land on `/app` (export.csv stays 403 text); five shadcn Selects (Dasbor outlet, Histori status, Buat kiriman pickup date/slot, Cek tarif outlet) rendered an empty trigger before hydration → `SelectValue` children carry the label, the pattern `laporan/_components/filter-select.tsx` already used. (2) `lowVolumeNote` (`src/lib/shipment-report-analytics.ts`, threshold `COURIER_LOW_VOLUME_THRESHOLD`) + `LowVolumeNote`: a courier (table and phone records), wilayah or route with fewer than 10 shipments shows a muted "Volume rendah (n = N)" under its name; the rates stay. Render test in `report-pages-render` (boundary 9/10, courier/province/route). Spec 18 layout counts 4 → 6. Evidence: tsc 0 (after `next typegen` refreshed stale `.next/types`); lint 0; full suite on the iso DB (BETTER_AUTH_URL 127.0.0.1:3110) 115 files / 1,345 passed / 2 failed — both `system-map-inventory`: layouts (fixed after the run, 6/6 layouts check green on re-run) and the action count, caused by the uncommitted `issueBatchInvoices` in `label/cetak/actions.ts` from other work in the worktree (not in spec 18 §9). Screenshots: session scratchpad `t236/`. Open: Radix Select SSR blank remains on admin/platform-only selects (status pull, Pengaturan outlet, platform filters, destination-area outlet); the route note wraps inside the narrow Rute teratas column.
