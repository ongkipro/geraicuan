...
- Previous period (`previousAnalyticsRange`): the equal-length span of WIB calendar days immediately before `start`, ending exactly where the current window begins. Per picker preset (T-240): Hari ini → Kemarin; Kemarin → the day before; 7/30 hari terakhir → the 7/30 days before; Bulan ini (N days elapsed) → the N days before the 1st (e.g. 1–26 Sep → 6–31 Agu), not the whole previous month and not 1–N of the previous month. `bulan-lalu` (URL-only since T-240) is the one exception: the complete calendar month before it (T-163, below). The comparison caption names the compared span.
...
| QUE-NEEDS-AWB | Siap dilanjutkan | QUE-ALL where `status IN ('DRAFT','ESTIMATED')`; the same predicate the `READY_TO_PROGRESS` queue uses. Supersedes ACT-DRAFT + ACT-ESTIMATED as the operator-facing surface after T-168 removed the dashboard snapshot block | tenant | no tile since T-238 (owner 2026-09-26): the `READY_TO_PROGRESS` filter stays in the Histori status select and the Dasbor links | Retired tile |
...
| QUE-CANCELLED | Dibatalkan | QUE-ALL where `status = 'CANCELLED'` (Mengantar reported the order cancelled, T-238); never part of QUE-ATTENTION | tenant | /app/pengiriman panel (`?status=CANCELLED`) | Aligned |
...
| LBL-CANCELLED | Dibatalkan | issued resi (order `ISSUED`, AWB present, same suffix and issued-basis period) whose shipment is now `CANCELLED` (T-238); never in LBL-ALL, never printable | tenant | /app/label panel (`?cetak=batal`) | Aligned |
...
| QUE-SHARE | "x%" on each Histori status segment (not Semua kiriman) + its width in the strip's composition bar (T-248) | `round(100 × tile count / QUE-ALL)`, 0 when QUE-ALL = 0; every QUE-* tile is a subset of QUE-ALL. The page passes QUE-ALL as the base (`StatusTiles total`, `tileShare`), T-247 | tenant | /app/pengiriman panel | Aligned |
| RTS-SHARE | "x%" on each Retur status segment (not Semua retur) + its width in the composition bar (T-248) | `round(100 × tile count / RTS-ALL)`, 0 when RTS-ALL = 0; every RTS-* tile is a subset of RTS-ALL, T-247 | tenant | /app/pengiriman/rts panel | Aligned |
| LBL-SHARE | "x%" on each Cetak resi status segment (not Semua resi) + its width in the composition bar (T-248) | `round(100 × tile count / (LBL-ALL + LBL-CANCELLED))`, 0 when the base is 0 (`labelTileShareBase`). LBL-CANCELLED is never inside LBL-ALL, so the base is every issued resi in scope; the share was once taken of the largest tile, which let Dibatalkan exceed 100 % (review 2026-09-26, T-247) | tenant | /app/label panel | Aligned |
| QUE-OTHER | "Lainnya (n)" + grey segment of the Histori composition bar | QUE-ALL − (QUE-AWAITING-PICKUP + QUE-IN-TRANSIT + QUE-DELIVERED + QUE-CANCELLED + QUE-ATTENTION): the stages without a tile (DRAFT, ESTIMATED, SUBMISSION_QUEUED, RTS_*). The five tile predicates are disjoint status sets (tested), so it is never negative; `compositionSegments` clamps at 0, T-248 | tenant | /app/pengiriman panel | Aligned |
| RTS-OTHER | "Lainnya (n)" in the Retur composition bar | RTS-ALL − (RTS-QUEUED + RTS-IN-TRANSIT + RTS-RECEIVED + RTS-PROBLEM); 0 by construction: RTS-ALL is the sum of exactly those four status buckets, so no segment is drawn, T-248 | tenant | /app/pengiriman/rts panel | Aligned |
| LBL-OTHER | "Lainnya (n)" in the Cetak resi composition bar | (LBL-ALL + LBL-CANCELLED) − (LBL-UNPRINTED + LBL-PRINTED + LBL-CANCELLED); 0 by construction: LBL-ALL = LBL-UNPRINTED + LBL-PRINTED, so no segment is drawn, T-248 | tenant | /app/label panel | Aligned |
...
**Contact shipment figures (T-241).** Pengirim/Penerima list rows and detail pages show what GeraiCUAN's own shipments say about a contact — never a Mengantar score (D-30). A shipment belongs to a contact by DATA-19's attribution rule (same tenant, the page's party role, same national phone number). Lifetime snapshots, no period basis. Read by `src/db/contact-shipment-repository.ts`; verified by `tests/contact-numbers-t241.integration.test.ts`.

| ID | Label | Definition | Scope | Surface |
|---|---|---|---|---|
| CON-SHP-COUNT | Kiriman · "N kiriman" / Total kiriman | `count(*)` of attributed shipments, any status (SHP-CREATED's basis); "—" in the list when 0 | contact × role | list column Kiriman; detail KPI Total kiriman |
| CON-SHP-DELIVERED | (numerator) | attributed shipments with `status = 'DELIVERED'` (SHP-OUTCOME-DELIVERED's predicate) | contact × role | — |
| CON-SHP-DELIVERED-SHARE | "x% terkirim" | CON-SHP-DELIVERED ÷ CON-SHP-COUNT × 100, at most one decimal; not shown when CON-SHP-COUNT = 0 | contact × role | list column Kiriman; detail KPI note |
| CON-SHP-COD-VALUE-IDR | Nilai COD | Σ `shipment_cod_totals.provider_cod_amount_idr` over attributed shipments whose provider order is `is_cod` and `ISSUED` — RPT-SHP-COD-VALUE-TOTAL's join; a liability passing through Mengantar, never revenue | contact × role | detail KPI |
| CON-SHP-COD-COUNT | "dari N kiriman COD yang resinya terbit" | count of the orders behind CON-SHP-COD-VALUE-IDR | contact × role | detail KPI note |
| CON-SHP-RETURNED | (numerator) | attributed shipments with `status IN RTS_STATUSES` (SHP-OUTCOME-RETURNED's predicate) | contact × role | detail KPI note |
| CON-SHP-RETURN-RATE | Tingkat retur | CON-SHP-RETURNED ÷ (CON-SHP-DELIVERED + CON-SHP-RETURNED) × 100 (RPT-SHP-RETURN-RATE's formula), "—" when none has settled | contact × role | detail KPI |
| CON-SHP-LAST-30D | Kiriman 30 hari terakhir | attributed shipments with `created_at >= statement_timestamp() − 30 days` | contact × role | detail KPI |
| CON-SHP-HISTORY-ALL / -COD / -NONCOD | Riwayat kiriman tabs Semua / COD / Non-COD | CON-SHP-COUNT split by the draft's `is_cod` (SHP-COD's basis: COD Ongkir is COD); -NONCOD = -ALL − -COD | contact × role | detail history tabs |
...
| RPT-SHP-LIFECYCLE-COUNT | Distribusi status · Kiriman | RPT-SHP-ROWS grouped by `shipments.status`; since T-254 shown inside its RPT-SHP-LIFECYCLE-GROUP bucket, in lifecycle order (`shipmentStatuses`), each with its `ShipmentStatusBadge`, the count and its share of RPT-SHP-ROWS (one decimal) |
| RPT-SHP-LIFECYCLE-GROUP | Distribusi status · bucket (T-254) | Σ RPT-SHP-LIFECYCLE-COUNT over the statuses of one Ringkasan outcome bucket (`reportStatusGroups`; a `Record` over every status): `DELIVERED` → Terkirim, `RTS_*` → Retur, `FAILED` + `CANCELLED` → Gagal, the rest → Masih berjalan. Equals RPT-SHP-DELIVERED / -RETURNED / -FAILED / -IN-PROGRESS for the same cohort; share = bucket ÷ RPT-SHP-ROWS, one decimal. No new number |
| RPT-SHP-COURIER-DELIVERED | Total per kurir · (numerator of % terkirim) | count in that courier group with `status = 'DELIVERED'` — CRR-DELIVERED's predicate |
| RPT-SHP-COURIER-RETURNED | Total per kurir · (numerator of % retur) | count in that courier group with `status IN RTS_STATUSES` — CRR-RETURNED's predicate |
| RPT-SHP-COURIER-DELIVERED-RATE | Total per kurir · % terkirim | RPT-SHP-COURIER-DELIVERED ÷ RPT-SHP-COURIER-COUNT × 100, one decimal; "—" when the group is empty |
| RPT-SHP-COURIER-RETURN-RATE | Total per kurir · % retur | RPT-SHP-COURIER-RETURNED ÷ (RPT-SHP-COURIER-DELIVERED + RPT-SHP-COURIER-RETURNED) × 100, one decimal; "—" when nothing in the group has settled |

**Analytics above the list (T-235, 2026-09-26).** Owner: "laporan pengiriman mungkin kurang lengkap, tambahkan analitik, diagram jika perlu". `loadShipmentReportAnalytics` reads **the same cohort** as the rows, totals and CSV — `reportJoins` + `reportFilter`, the table's own `tenant_id` predicate, Tenant Admin only — so every figure below follows the period, outlet, kurir and status filters and reconciles with RPT-SHP-ROWS. Aggregated in SQL (one KPI row, one bucket per WIB day/month, one row per outlet × stored area label); only the area label is parsed in TypeScript, after grouping, by the one parser `parseAreaRegion` (`src/lib/label-format.ts`), folded by `groupRegions` / `groupRoutes` (`src/lib/shipment-report-analytics.ts`). A failed analytics read degrades to one alert; totals and the list stay.

| ID | Label | Definition | Basis |
|---|---|---|---|
| RPT-SHP-KPI-TOTAL | Total kiriman | RPT-SHP-ROWS | `shipments.created_at` (cohort) |
| RPT-SHP-DELIVERED | Terkirim | cohort with `status = 'DELIVERED'` — **SHP-OUTCOME-DELIVERED's predicate** | cohort, current status |
| RPT-SHP-DELIVERED-SHARE | Terkirim · % | RPT-SHP-DELIVERED ÷ RPT-SHP-ROWS × 100, one decimal, "dari N kiriman" | as above |
| RPT-SHP-RETURNED | Retur | cohort with `status IN RTS_STATUSES` (`RTS_QUEUED`, `RTS_IN_TRANSIT`, `RTS_RECEIVED`; never `PROBLEM`) — **SHP-OUTCOME-RETURNED's predicate** | as above |
| RPT-SHP-RETURN-RATE | Retur · % | RPT-SHP-RETURNED ÷ (RPT-SHP-DELIVERED + RPT-SHP-RETURNED) × 100, one decimal, "dari N selesai"; "—" when none has settled. "Selesai" = the outcome is known (terkirim or retur); a return still queued counts as settled because its outcome is decided | as above |
| RPT-SHP-FAILED | Gagal | cohort with `status IN ('FAILED','CANCELLED')` — SHP-OUTCOME-FAILED's predicate (T-238: a Mengantar-cancelled shipment is neither delivered nor returned). Shown since T-251 (count only, no rate); before that only subtracted for Masih berjalan | as above |
| RPT-SHP-IN-PROGRESS | Masih berjalan | RPT-SHP-ROWS − RPT-SHP-DELIVERED − RPT-SHP-RETURNED − RPT-SHP-FAILED — **the Dasbor's "Masih berjalan" row exactly** (drafts, estimates, queued, issued, in transit, problem, awaiting payment, unknown) | as above |
| RPT-SHP-OUTCOME-COMPOSITION | Ringkasan · composition bar (T-251) | RPT-SHP-DELIVERED, RPT-SHP-RETURNED, RPT-SHP-FAILED and RPT-SHP-IN-PROGRESS as stacked segments, each width = count ÷ RPT-SHP-ROWS. The three outcome predicates are disjoint status sets and Masih berjalan is the remainder, so the four sum to RPT-SHP-ROWS exactly, with no "Lainnya" segment. No new number: the bar repeats the four counts beside it (`reportOutcomeComposition`). No estimasi-vs-potongan bar: RPT-SHP-COD-VALUE-TOTAL − RPT-SHP-COD-DISBURSEMENT-EST-TOTAL is not a defined cohort metric. The per-row identity holds, but the report's shipping and fee sums also cover non-COD rows, and a COD Ongkir estimate can be negative | as above |
| RPT-SHP-COD-VALUE-TOTAL | Nilai COD | Σ `shipment_cod_totals.provider_cod_amount_idr` (the canonical COD amount, T-199) over cohort rows whose provider order `is_cod` — what couriers collect; drafts and non-COD orders contribute nothing, a stale COD total on a non-COD order is excluded by the join. Caption carries the count of those orders (`codOrderCount`). A liability passing through Mengantar (FIN-COD-PRINCIPAL), never revenue | cohort |
| RPT-SHP-COD-DISBURSEMENT-EST-TOTAL | Estimasi cair | Σ RPT-SHP-COD-DISBURSEMENT-EST-IDR over the cohort = Σ RPT-SHP-COURIER-COD-DISBURSEMENT-EST-IDR (the same figure the "Total per kurir" column sums) | cohort |
| RPT-SHP-TREND-COD / RPT-SHP-TREND-NONCOD | Tren harian · COD / Non-COD | RPT-SHP-ROWS per `date_trunc('day', created_at AT TIME ZONE 'Asia/Jakarta')` (`'month'` above 31 days, M-0), split on `shipment_drafts.is_cod` — **SHP-COD's basis, COD Ongkir included**, the Dasbor's split. Every bucket of the range is drawn; an empty day is 0 | `shipments.created_at`, WIB |
| RPT-SHP-TREND-COD-VALUE | Tren harian · Nilai COD per hari | RPT-SHP-COD-VALUE-TOTAL per the same bucket | as above |
| RPT-SHP-TREND-TOTALS | Tren harian · legend totals (T-254) | Σ over every bucket of RPT-SHP-TREND-COD ("COD n kiriman"), RPT-SHP-TREND-NONCOD ("Non-COD n kiriman") and RPT-SHP-TREND-COD-VALUE ("Nilai COD Rp …") (`reportTrendTotals`). Every bucket of the range is drawn, so COD + Non-COD = RPT-SHP-ROWS and the value = RPT-SHP-COD-VALUE-TOTAL. The COD count is on the draft flag (COD Ongkir included), so it can differ from RPT-SHP-COD-VALUE-TOTAL's caption count, which counts issued COD orders | as above |
| RPT-SHP-REGION-PROVINCE / RPT-SHP-REGION-CITY | Wilayah tujuan · Per provinsi / Per kota | RPT-SHP-ROWS grouped by the province / (city, province) read from `shipment_drafts.destination_area_label` ("subdistrict, district, city, province, zip"): drop a trailing five-digit postal code, the last part is the province, the one before it the city — counted from the end exactly as `formatDistrictCity`. Grouping is case- and spacing-blind ("KOTA MAKASSAR" = "Kota Makassar"); display through `areaDisplayCase`. A label with fewer than three area parts is **"Wilayah tidak dikenal"**, always listed last. Columns: Kiriman, Terkirim (RPT-SHP-DELIVERED in the group), Retur (RPT-SHP-RETURNED in the group), % retur (RPT-SHP-RETURN-RATE in the group). Sorted by volume; top 10 in the table and bar chart, the rest behind "Tampilkan semua"; the groups always sum to RPT-SHP-ROWS | cohort |
| RPT-SHP-ROUTE-COUNT | Rute teratas | RPT-SHP-ROWS grouped by (outlet, destination city as in RPT-SHP-REGION-CITY), top 5 by volume, with % terkirim (RPT-SHP-DELIVERED-SHARE in the route) and % retur. Routes into "Wilayah tidak dikenal" are left out (not actionable), so this table does not sum to RPT-SHP-ROWS by design | cohort |

**Dasbor alignment.** For the same period and outlet with no kurir/status filter, Total kiriman = SHP-CREATED, Terkirim / Retur / Masih berjalan = the Dasbor "Hasil pengiriman" totals (SHP-OUTCOME-*), and the trend's COD / Non-COD = SHP-COD / SHP-NONCOD — same predicates, same cohort basis, same WIB buckets. Verified on the dev tenant 2026-09-26 (30 hari: 134 / 45 / 18 / 65 on both pages). Per-courier % terkirim / % retur use CRR-DELIVERED / CRR-RETURNED's predicates; the Dasbor recap omits unbatched shipments, the report keeps them as "Belum ada kurir".

**Verification (T-235).** `tests/shipment-report.integration.test.ts` "shipment report analytics (T-235)": KPIs equal the fixture-absolute values and RPT-SHP-ROWS and Histori's count, Estimasi cair equals the per-courier sum, trend buckets per WIB day on the draft flag, provinces/cities casing-blind with the unknown group last and summing to the cohort, routes skipping the unknown, and every filter reconciling with the list; the tenant-predicate test now also covers the analytics statements. `tests/report-pages-render.integration.test.ts` covers the parser, the grouping, the rate formatting, the filled buckets and the rendered sections. **T-251:** `tests/shipment-report` "splits the total into disjoint outcome buckets that sum exactly" seeds a FAILED and a CANCELLED shipment and binds each bucket to an independent per-status count (RPT-SHP-LIFECYCLE-COUNT). `tests/report-pages-render` pins each `<dt>`/`<dd>` pair, the segment order, the counts summing to the total and the widths summing to 100 %. **T-254:** `tests/shipment-report` binds RPT-SHP-LIFECYCLE-GROUP to the Ringkasan buckets (same seeded FAILED + CANCELLED cohort) and RPT-SHP-TREND-TOTALS to RPT-SHP-ROWS and RPT-SHP-COD-VALUE-TOTAL. `tests/report-pages-render` places every status in exactly one bucket in lifecycle order, pins the legend `<dt>`/`<dd>` totals and the grouped rows.
...
**Formula version 3 (migration `0050_cod_ongkir_payment_method`, 2026-09-17).** The goods were paid for outside GeraiCUAN; the courier collects a shipping charge `C` chosen by the operator when the service is confirmed. Mengantar keeps the shipping it deducts `S` plus exactly `0.0333 × C` (the same evidence as PR-9 above), so the seller nets `C − S − 0.0333·C`. The COD Ongkir charge was **computed as a floor, typed only upward** until D-28 (2026-09-26, T-237): since then a new row always records exactly COD-ONGKIR-BREAK-EVEN-IDR (ongkir + biaya COD), computed server-side (`computedCodOngkirChargeIdr`), never typed; rows recorded earlier with a raised charge stay valid under the same version 3 checks.
...
| COD-ONGKIR-CHARGE-IDR | Nilai COD Ongkir (ongkir + biaya COD) | `provider_cod_amount_idr` of a version 3 row; since D-28 exactly COD-ONGKIR-BREAK-EVEN-IDR for a new row (`codOngkirAmount` on screen, `computedCodOngkirChargeIdr` on the server, `NOT_COMPUTED` otherwise); below it refused by `calculateCodOngkirAmounts` and by the database | Aligned |
...
| COD-ONGKIR-SELLER-DIFFERENCE-IDR | Selisih diterima penjual | `C − S − COD-ONGKIR-MENGANTAR-FEE-IDR` (`codOngkirSellerDifferenceIdr`); ≥ 0 for every accepted charge, rounding included. Since D-28 it equals the "Pembulatan" line (0 or a few rupiah) | Aligned |
...
| /app/laporan/pengiriman | How many shipments were created per day, COD vs Non-COD, and how much COD? | Line: RPT-SHP-TREND-COD (solid) vs RPT-SHP-TREND-NONCOD (dashed), or RPT-SHP-TREND-COD-VALUE, switched by Tabs; data table in a disclosure | M-1 / PR-55 | T-235 |
| /app/laporan/pengiriman | Where do shipments go? | Bar in the table (T-254, replaces the T-235 bar chart): top 10 RPT-SHP-REGION-PROVINCE / -CITY by count, an 8 px bar from zero under each name scaled to the largest known wilayah, the count in its own column; "Wilayah tidak dikenal" kept in the table without a bar | PR-55 | T-235, T-254 |
...
| SHP-OUTCOME-FAILED | Gagal | count of the cohort with `status IN ('FAILED','CANCELLED')` (T-238, owner 2026-09-26: "Dibatalkan" is reported as gagal; it is never in QUE-ATTENTION) | as above | /app | Aligned |
