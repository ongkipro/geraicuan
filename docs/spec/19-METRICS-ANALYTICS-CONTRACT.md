...
| QUE-SHARE | "x%" + bar on each Histori tile | `round(100 × tile count / QUE-ALL)`, 0 when QUE-ALL = 0; every QUE-* tile is a subset of QUE-ALL. The page passes QUE-ALL as the base (`StatusTiles total`, `tileShare`), T-247 | tenant | /app/pengiriman panel | Aligned |
| RTS-SHARE | "x%" + bar on each Retur tile | `round(100 × tile count / RTS-ALL)`, 0 when RTS-ALL = 0; every RTS-* tile is a subset of RTS-ALL, T-247 | tenant | /app/pengiriman/rts panel | Aligned |
| LBL-SHARE | "x%" + bar on each Cetak resi tile | `round(100 × tile count / (LBL-ALL + LBL-CANCELLED))`, 0 when the base is 0 (`labelTileShareBase`). LBL-CANCELLED is never inside LBL-ALL, so the base is every issued resi in scope; the share was once taken of the largest tile, which let Dibatalkan exceed 100 % (review 2026-09-26, T-247) | tenant | /app/label panel | Aligned |
...
| RPT-SHP-LIFECYCLE-COUNT | Distribusi status · Kiriman | RPT-SHP-ROWS grouped by `shipments.status`; since T-235 drawn as bars in lifecycle order (`shipmentStatuses`), each with its `ShipmentStatusBadge`, the count and its share of RPT-SHP-ROWS (one decimal) |
...
| RPT-SHP-FAILED | — (not shown; subtracted for Masih berjalan) | cohort with `status IN ('FAILED','CANCELLED')` — SHP-OUTCOME-FAILED's predicate (T-238: a Mengantar-cancelled shipment is neither delivered nor returned) | as above |
...
| /app/laporan/pengiriman | Where do shipments go? | Horizontal bar, top 10 RPT-SHP-REGION-PROVINCE / -CITY by count, count beside each bar, unknown excluded from the chart but kept in the table | PR-55 | T-235 |
