...
| Pickup time | `POST /time` (form-urlencoded, `MM-DD-YYYY`, ≥ 90 min, 09:00–18:00) | none. GeraiCUAN offers its own 08.00–17.00 WIB slots (starts 08–16, owner T-234, D-25); the 08:00 slot is a GeraiCUAN-side schedule outside Mengantar's documented window. DB CHECK `shipment_drafts_pickup_slot_valid` (0061) accepts `^(0[89]\|1[0-7]):00$`: 17:00 is legacy (pre-T-234 drafts) | D (AdsBookCMS stub only), **unverified** | after T-153: confirm the real window before sending any slot; an 08:00 slot may need mapping or refusal then |
