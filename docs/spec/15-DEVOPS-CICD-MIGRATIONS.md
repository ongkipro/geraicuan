...
| ops image (one-off jobs) | `docker build --target ops` of the release commit, on the Coolify server (DEP-3) | — | the `deps` stage (all dependencies, `drizzle-kit` included) plus `drizzle/`, `drizzle.config.ts`, `scripts/bootstrap-super-admin.mjs`, `scripts/wilayah-import.mjs`, `src/lib/wilayah.ts`, `data/wilayah/`; user `node` | default `pnpm db:migrate`; also `pnpm wilayah:import` (after migrate, T-245) and `pnpm ops:bootstrap-super-admin` |
...
**Wilayah reference import (T-245, D-32, DATA-22).** Migration `0067_wilayah_reference` runs `CREATE EXTENSION IF NOT EXISTS pg_trgm` and creates the empty, tenant-neutral `wilayah_areas`. **Prerequisite and release gate:** `pg_trgm` must be available on the production Coolify PostgreSQL and the migration role must be allowed to create it (PostgreSQL 13+ marks it trusted, so a role with CREATE on the database suffices; otherwise the superuser runs `CREATE EXTENSION pg_trgm;` once before migrating). Check as the superuser before the deploy: `SELECT name, default_version, installed_version FROM pg_available_extensions WHERE name = 'pg_trgm';` must return a row. Not yet confirmed by the owner (RELEASE.md). If it is refused, stop: the fallback is a changed migration without the trigram index (the same `LIKE` queries then scan the ~91k rows; measure before accepting it). After `pnpm db:migrate`, run the import in the same ops container, same migration-role env file:
```bash
docker run --rm --network coolify --env-file /root/geraicuan-migrate.env geraicuan-ops:<commit> pnpm wilayah:import
```
It is offline (reads `data/wilayah/` from the image), verifies the vendored SHA-256 checksums and published counts before writing, replaces the rows in one transaction, asserts the row count and content fingerprint before COMMIT, then `ANALYZE`s the table, and prints one JSON line (counts, added/changed/removed, md5, sizes). Expect `"rows":91047` and, on a re-run, `"added":0,"changed":0,"removed":0`. Until it has run, suggestions are empty and the picker offers "Cari langsung di Mengantar" (the provider search), so the app works in the gap. **Refresh** when upstream publishes a new decree: new pinned checkouts of `cahyadsn/wilayah` and `cahyadsn/wilayah_kodepos` → `node scripts/wilayah-vendor.mjs <wilayah> <kodepos> "<decree>"` → review the `data/wilayah/` diff and `SOURCE.json` → commit → import; record the import line in `BUILD-LOG.md`. The ops image copies `scripts/wilayah-import.mjs`, `src/lib/wilayah.ts` and `data/wilayah/` (Node 22.18+ strips the TypeScript types at run time).
...
| `MENGANTAR_WEBHOOK_ENABLED` | app | Optional; leave unset until the Mengantar account's webhook is enabled (D-30). | `1` opens `POST /api/webhooks/mengantar` on the tenant host; anything else keeps it an empty `404` that reads nothing. Needs `MENGANTAR_WEBHOOK_SECRET` too. Register `https://app.geraicuan.com/api/webhooks/mengantar` in Mengantar under Settings → Account → Integrasi Public API. | `1` |
| `MENGANTAR_WEBHOOK_SECRET` | app | Optional, secret; required only with `MENGANTAR_WEBHOOK_ENABLED=1` (blank keeps the route closed). | The Webhook Secret Mengantar shows for the **platform-default** account: the HMAC-SHA256 key of `{x-timestamp}.{raw body}` (T-238). Deliveries for a store's private Mengantar account are not accepted with it. Rotating it in Mengantar requires the same value here. | `<mengantar-webhook-secret>` |
...
Not in the table because no deployed code reads them: `PORT` and `HOSTNAME` (read by the standalone `server.js`; the Dockerfile sets `3000` and `0.0.0.0`, and the image `HEALTHCHECK` reads `PORT`; leave both unset); `BETTER_AUTH_SECRETS` / `AUTH_SECRET` (Better Auth alternatives; use `BETTER_AUTH_SECRET` only); `ASTRO_TELEMETRY_DISABLED` (set inside `apps/landing/package.json` scripts); `MIGRATION_CHECK_DATABASE_URL` and `POSTGRES_*` (CI and local tooling only).

Summary per service:
- `geraicuan-app` build (Dockerfile build arguments; the defaults are the production values; set them as build variables so a change is visible, always equal to runtime): `BETTER_AUTH_URL`, `BETTER_AUTH_TRUSTED_ORIGINS`, `BETTER_AUTH_TRUSTED_PROXY_CIDRS`, `GERAICUAN_TENANT_ORIGIN`, `GERAICUAN_PLATFORM_ORIGIN`, `GERAICUAN_PUBLIC_ORIGIN`. Runtime: those six plus `APP_DATABASE_URL`, `BETTER_AUTH_SECRET`, `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, `MENGANTAR_CREDENTIAL_ENCRYPTION_KEY`, optionally the four platform-default `MENGANTAR_*` values, and — only once the account's webhook is enabled — `MENGANTAR_WEBHOOK_ENABLED` and `MENGANTAR_WEBHOOK_SECRET`.
