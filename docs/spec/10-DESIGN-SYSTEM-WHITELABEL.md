...
## Visual Direction
- Public sales page: **since T-184 (D-11, PR-63) the static Astro site in `apps/landing`, served on `geraicuan.com`**; `src/app/page.tsx` is no longer the public page in production. Brand/Marketing mode: light canvas, dark ink, the one blue accent, hairline-led surfaces, and two intents — register (`Daftarkan toko` → `<tenant origin>/daftar`) and sign in (`Masuk` → `<tenant origin>/login`). It links to no Super Admin surface. Dark mode is out of scope for MVP.
- Public tokens are shared, not re-invented: `apps/landing/src/styles/landing.css` carries a verbatim copy of the `src/app/globals.css` `:root` values it uses (`--primary`, `--primary-hover`, `--ring`, `--ok`, `--muted`, `--muted-foreground`, `--border`, `--radius`, the Inter/mono font stacks and the rest listed there), the same Inter family self-hosted at build, and the CMS focus contract (`:focus-visible` 2px full-alpha `--ring`, offset 2px). `scripts/ui-audit/landing-page.mjs` check `token-parity` fails when any copied value drifts from `globals.css`; change the CMS token first, then copy. The site is Indonesian, readable for operators aged 40+ (18px body text, 44px targets at every width), zero client JavaScript.
...
- Public signature: a 10 × 15 cm thermal label rendered in HTML/SVG from sanitized sample data (package label, cut line, sender stub, a real Code 128 symbol of a fictional AWB) makes the printed output, the COD amount and the handover stub tangible without screenshots or product mockups. Below the hero the proof points are the size diagram (10 × 15 with stub, or 10 × 10), a worked COD example computed by the PR-9 formula, and the reporting capabilities. Do not repeat the framed label as a generic card pattern.
...
## Required surfaces
1. Public sales page (Astro, `apps/landing`, T-184): product explanation, how it works, the thermal label and COD precision proof points, reporting summary, and register/sign-in links to the tenant origin only; no public CMS data, application request, analytics tracker or operational action.
...
|---|---|---|---|
| Sales page (Astro) | Understand the product, then register or sign in on the tenant host | normal, unavailable CMS link | Never render operational data; CTA origin is the build-time `PUBLIC_APP_ORIGIN` only. |
