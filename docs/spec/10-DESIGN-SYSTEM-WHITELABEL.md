...
4. Tenant outlet configuration: outlet-scoped location readiness followed by `platform default` versus private Mengantar mode; a blank password field for creating/replacing an API key; static stored/not-verified/connected/attention state without key fragments; provider-authoritative area/pickup selectors when their contract is accepted; and safe connection errors without secret detail.
