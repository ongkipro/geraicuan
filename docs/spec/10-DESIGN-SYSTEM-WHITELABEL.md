...
- CMS Admin: clean, information-dense light operational surface using the repository's shadcn/Radix primitives and Tailwind semantic tokens. Since T-118 the interactive accent is the neutral shadcn primary (near-black on light, near-white on dark); neutral surfaces establish hierarchy; semantic green, amber, and red communicate status alongside text or icon cues. Use compact tables, explicit status badges, and no decorative dashboard cards where a queue or filterable table is the decision tool.
...
Historical status: T-109/T-110 recorded and implemented this direction locally. T-111/T-112 and then T-118 superseded its palette; the neutral T-118 section is the current colour authority. Retain these values as provenance, not an active implementation instruction. T-132 still requires an explicit replacement decision.
...
At mobile widths use the same DOM order as the accepted screen sequence. A desktop side region must not hide urgent work after a long table on mobile. Chart data tables remain visible on analysis workspaces; only command-center disclosures may begin collapsed under Pattern 1.
...
- On command-center routes only, the trend chart's complete semantic data table may start collapsed inside a native disclosure labelled with its row count. Analysis workspaces keep the table immediately below the chart, uncollapsed (UX-9).
...
- One filter bar (the accepted single GET form) with applied-filter chips, one shared StatCard summary row, charts each followed immediately by their uncollapsed data table, then the authoritative detail table. On `/app/keuangan` the UX-5 Finance order governs: summary strip → variance queue → optional variance chart → ledger table.
