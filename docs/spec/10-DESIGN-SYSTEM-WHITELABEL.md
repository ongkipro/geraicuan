...
One centered `Card` (max-width 400 px) on the canvas: brand mark, `CardTitle` 20 px, fields 44 px with labels above, password `InputGroup` with a show/hide icon button (`aria-pressed`), one full-width primary, secondary links as text links below. Errors: `FieldError` under the field + one `Alert` summary. Super Admin: same card on `--sidebar`-dark ground with a badge. Screen contract: spec 17 UX-v3.10.
