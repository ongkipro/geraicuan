...
| `/app/pengaturan/*` | `pengaturan.html` | Configure the gerai | Settings layout: sub-menu + content cards (Profil gerai: Identitas gerai with name read-only + WhatsApp field and one Simpan, prefix · Informasi label · Titik pickup list + add · Outlet readiness · Koneksi Mengantar radio cards) | Per card save | invalid WhatsApp, locked prefix, provider errors | `saveTenantContact`, `saveShipmentPrefix`, `saveLabelSettings`, pickup and credential actions |
| `/app/pengaturan/label` Informasi label | Mengantar analysis §9.12 | Choose what the thermal label prints, per size (PR-86) | Settings layout → one card: size option cards (Thermal 10 × 15 / 10 × 10) → two columns: switch rows (Alamat pengirim, No. telepon pengirim, Nama penerima, No. telepon penerima, Detail alamat penerima — off prints kota and provinsi only, Peringatan sebelum retur — replaces the issue time in the footer) · live preview of the real label sheet with example data (gerai name and WhatsApp as sender), scaled like the label page → footer Simpan (both sizes) | Simpan | saved / refused alert; defaults until saved; no switch for the Mengantar pickup identity (PR-71) | `saveLabelSettings`, `loadTenantLabelFields` |
...
4. **Administrasi**
   - **Pengaturan** → `/app/pengaturan` — Tenant Admin only. Its own menu holds Profil gerai (`/app/pengaturan`), Informasi label (`/app/pengaturan/label`), Titik pickup (`/app/pengaturan/pickup`), Outlet (`/app/pengaturan/outlet`), Koneksi Mengantar (`/app/pengaturan/koneksi`) and Anggota & akses (`/app/anggota`).
