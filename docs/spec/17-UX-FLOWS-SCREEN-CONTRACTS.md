...
| Shipment creation | Capture one valid shipment without losing data | Full-page grouped form with provider-authoritative destination search, visible progress, and action summary | pristine, destination loading/no-result/error, validation error, saved, estimating, blocked outlet | `Field`, `Input`, `Textarea`, `Command`, `Popover`, `RadioGroup`, `Alert`, `Button` |
