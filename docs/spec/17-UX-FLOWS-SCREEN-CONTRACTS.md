...
| Terbitkan / cetak ulang invoice (UX-v3.9) | ✓ | ✓ | — |
| Pengirim, Penerima (create, edit) | ✓ | ✓ | — |
| Arsipkan kontak (server `archiveContact` is admin-only) | — | ✓ | — |
| Cek resi, Cek tarif | ✓ | ✓ | — |
| Perbarui status dari Mengantar | — | ✓ | — |
| Laporan pengiriman (+ CSV), Riwayat cetak resi | — | ✓ | — |
| Pengaturan (profil incl. WhatsApp gerai, informasi label, prefix, pickup, outlet, koneksi), Anggota | — | ✓ | — |
...
2. **Cek tarif**: services from the estimate snapshot appear in section 5 on the same page (courier logo grid → service list). Failure: message + "Coba lagi"; the draft stays. Offered services (T-237): Ninja never (D-29, removed: no catalogue entry and no logo; a historical Ninja shipment shows the plain text "Ninja"); cargo services (JNECargo, SiCepatCargo, SapCargo, iDexpressCargo) under their courier's logo, priced from the cargo tier and hidden below `minimumWeightCargo` (SapCargo: 5 kg); COD marked "Tidak" when `unsupported_cod`, `coverage_cod: false` or `unsupportedCodCheckFirstSap` (D-30) says so. SPX and SAP Lite stay quotable but a confirmation is refused with "belum bisa dipesan lewat API Mengantar" (no documented order value). The same list rules apply to `/app/cek-tarif`.
...
5. **Invoice** (UX-v3.9): **Cetak resi + invoice** issues the nota once and prints both; either can be reprinted alone.
6. **Follow**: Tenant Admin runs **Perbarui status dari Mengantar** (`pullMengantarStatus`) on Histori/Retur; statuses move; Retur lists returns.
...
| `/app/pengiriman/baru` Buat kiriman | `buat-kiriman.html` | Create a shipment and issue its resi | Header → stepper card (Isi data · Cek tarif · Terbitkan resi) → two columns: **form** [1 Penyerahan & asal: cards Penjemputan terjadwal / Drop di outlet, pickup address block from the titik pickup with "Ubah", pickup date + slot (shadcn Selects with calendar/clock icon; 09.00–18.00 WIB, starts 09.00–17.00, ≥ 90 min ahead today; D-27, T-237 — a saved 08.00 draft still shows its slot), note "Belum dikirim ke Mengantar" · 2 Pengirim & penerima: sub-block DATA PENGIRIM (CETAK DI LABEL) with badge MASKING and "Gunakan masking pengirim" (off = gerai identity + pickup address; on = editable name, phone, kota; "Cari kontak tersimpan"), sub-block DATA PELANGGAN (name, phone, full address, the shared destination-area picker — local kecamatan/kelurahan/kode pos suggestions, one Mengantar lookup, T-245) · 3 Pembayaran: cards COD (BAYAR DI TEMPAT, 3,33%) / Non-COD (TRANSFER/LUNAS); COD value row with checkbox "COD Ongkir" and fee line · 4 Produk & paket: rows Nama · Jumlah · Berat (kg), dashed "Tambah produk", Instruksi (opsional), "Detail tambahan" disclosure (dimensi, barang berbahaya) · 5 Pilih layanan ekspedisi: before estimate an empty frame "Tarif muncul setelah data disimpan" (locked: lock marker, "Terbuka setelah cek tarif"); after, courier logo grid + service list + "Paket sudah dicek fisik"]; T-249: every section shows its state (complete / current / pending / locked marker, "Lengkap" or "n isian belum diisi", counted from the required fields only), on a progress spine from 1024px, and step 1 of the top bar reads "n/4 bagian lengkap" + **rail** "Ringkasan tagihan", never taller than the window (spec 10 §4.8): header (title, ESTIMASI/TARIF RESMI badge, "Langkah pengisian" row of five section links + "n/4 lengkap") · scrolling body (Asal · Tujuan one line each, rows incl. "Pengirim di label" with phone, rincian komponen biaya) · pinned footer (big total, primary, guard "Belum lengkap: <sections>" or "Simpan draf"); below 1024px the bottom bar (n/4 bagian lengkap, total, Rincian → bottom Sheet with the full summary, primary) | Simpan & cek tarif → Konfirmasi & terbitkan AWB (one visible at a time) | field errors with summary, estimate loading/failure, issuance pending/outcomes, gerai pending approval (read-only notice) | `saveShipmentDraft`, `searchSender/RecipientShipmentContacts`, `selectShipmentContact`, `searchWilayahDestinationAreas`, `resolveWilayahDestinationArea`, `searchMengantarDestinationAreas`, `verifyShipmentDraftDestinationArea`, `loadShipmentEstimate`, `confirmShipmentIssuance` |
| `/app/pengiriman` Histori kiriman | `histori-kiriman.html` | Find a shipment and act on it | Header (Buat kiriman primary) → filter row → status tiles (Semua, Siap dilanjutkan, Resi terbit, Dalam perjalanan, Terkirim, Perlu perhatian) → one card: toolbar (search `cari` = nomor kiriman/resi only — no name/phone because the term is in the URL (spec 10 §11); it narrows tiles and list alike, "Perbarui status dari Mengantar" admin outline + one freshness line) → table (Nomor · Status/Pembayaran · Penerima · Paket/Outlet · Ekspedisi/Resi · Aktivitas) → footer count + pagination | Buat kiriman / Perbarui status | empty, filtered-empty, stale, pull result (status/alert only after the action) | `shipment-queue-repository`, `pullMengantarStatus`, `loadProviderDeliveryStatusBasis` |
...
| `/app/label/[n]` Label | `label-detail.html` | Print the masked label (and invoice) | Back link → header (Label <resi>) → left: size cards (10×15 default, 10×10) + **Cetak label** (primary) and outline **Cetak resi + invoice** (→ `?invoice=1`) + riwayat cetak; with `?invoice=1`: primary **Cetak resi + invoice** until the invoice exists, then ordered steps "1 · Label termal" and "2 · Invoice" (the next unfinished step is the primary), outline **Cetak label saja** / **Cetak invoice saja**; right: preview | Cetak label / Cetak resi + invoice | not issued, unpaid | `recordLabelPrint`, `issueShipmentInvoice`, label sheet |
| `/app/label/cetak` Cetak massal | `riwayat-pesanan` print modal (Mengantar §9.5) | Print several labels and/or invoices at once (PR-87) | Reached only from the Cetak resi modal (`n`, `ukuran`, `isi`) → skipped list with reasons → sheets in order with page breaks | Cetak (records one print per shipment) | invalid query, all skipped | `recordBatchLabelPrints`; issues missing invoices idempotently on render when `isi` includes invoices (explicit user choice in the modal; `lazy:` could move to the modal action) |
| `/app/invoice/[n]` Invoice | `label-detail.html` (layout) | Print or reprint the nota alone | Back link → header (Invoice INV-GC-10058) → left: media cards (80 mm default, A4) + **Cetak invoice** ; right: invoice preview | Cetak invoice | not issued (→ link to detail), invoice absent (issue on first open is **not** automatic: button "Terbitkan invoice") | `issueShipmentInvoice`, invoice sheet |
| `/app/kontak/pengirim`, `/penerima` | `pengirim.html`, `penerima.html` | Reuse parties | Header (Pengirim/Penerima baru primary) → card: tabs Aktif/Diarsipkan/Semua with counts + live search (term never in the URL) → table (Nama as link + kategori badge + "Juga …" · WhatsApp · Alamat utama · Kiriman "N kiriman / x% terkirim" or "—" · Status Aktif/Diarsipkan · Aksi WhatsApp/salin/Detail) → pagination (20); below `md` record cards (T-241) | New contact | empty, no results | `contact-repository`, `contact-shipment-repository`, `searchContacts` |
| `/app/kontak/baru` | `kontak-baru.html` | Create a contact | Back link → header → cards Kontak (nama, telepon, Peran / kategori optional) · Peran · Alamat pertama | Simpan kontak (opens `/app/kontak/<peran>/<n>`) | validation | `saveContact` |
| `/app/kontak/pengirim/<n>`, `/app/kontak/penerima/<n>` | `pengirim-detail.html`, `penerima-detail.html` | Read and edit one contact (T-241) | Back link → header (name, kategori badge, Aktif/Diarsipkan, "N alamat terdaftar"; WhatsApp line and wilayah = city, province of the primary address; WhatsApp · Salin nomor · primary **Buat kiriman dari kontak ini**) → 4 KPI cards from GeraiCUAN data only (Total kiriman + % terkirim · Nilai COD · Tingkat retur · Kiriman 30 hari terakhir; spec 19 CON-SHP-*; no Mengantar score) → **T-246: the page is one white surface; only the KPI row stays as (bordered) cards, every region below is a flat section with a hairline above it (spec 10 §4.11a)** → **T-250 (owner 2026-09-26, "bisa kau buat 2 colum"): below the KPI row, two columns once the content column is ≥ 896px (container query `@4xl/detail`, so 1280/1440 with the full sidebar, and 1024 only with the sidebar collapsed to its rail): main column `minmax(0,1fr)`, side column 340px, 48px whitespace between them, no card and no vertical rule.** Main column: Riwayat kiriman (tabs Semua/COD/Non-COD with counts; four columns — Resi, ekspedisi & tanggal (GC link, courier logo + resi, date · time WIB under it) · counterpart & kota tujuan · Status · Pembayaran — no horizontal scroll at 1440/1280/1024; phone keeps the record-card list; 10 per page; rows link to the shipment), then Alamat (count + Tambah alamat; bordered white items, Utama badge first, Jadikan utama, Edit/Tambah dialogs; one address full width, two or more 2-up once the section is ≥ 512px). Side column (sticky 96px from the top when the viewport is ≥ 896px tall): Data kontak (nama, telepon, kategori stacked; Peran as two compact 44px checkbox rows whose effect text stays as the checkbox description for assistive technology; one full-width "Simpan data kontak" — `updateContactAction` unchanged, it always saved identity and roles together; archived: read-only facts), then Zona hati-hati, a quiet section with a destructive-outline Arsipkan behind the confirmation (Tenant Admin only). The DOM follows the desktop reading order (main, then side); below 896px the column wrappers become `display: contents` and `order` stacks Data kontak · Alamat · Riwayat kiriman · Zona hati-hati (independent sections, so the visual reorder does not change meaning). The loading skeleton takes the same shape | Buat kiriman dari kontak ini (links `/app/pengiriman/baru`; no preselect yet) | archived (read-only), no shipments, wrong role → the role it holds, keeping `riwayat`, `halaman` and the `tersimpan`/`diarsipkan` notice (T-247), unknown/other-tenant number → not found | `getContactByNumber`, `contact-shipment-repository`, `updateContactAction`, `add/updateContactAddressAction`, `setPrimaryContactAddressAction`, `archiveContactAction` |
| `/app/kontak/[id]` (legacy) | — | Old bookmarks | Permanent redirect to `/app/kontak/<peran>/<n>` (`dari` kept when the contact still holds it); another tenant's or malformed id → not found | — | — | `getContact` |
| `/app/cek-resi` | `cek-resi.html` | Track by number or resi | Header + description → card: input (label above, helper) + **Cek resi** → idle placeholder / pending skeleton / result card (number, resi mono + Salin, status badge, fact tiles Ekspedisi · Pembayaran · Tujuan, newest-first timeline with source, "Diperbarui … WIB" + Lihat rincian kiriman) + tips rail (T-242) | Cek resi | idle, pending, found, not found, invalid, rate limited, unavailable, route error | `lookupShipmentTracking` |
| `/app/cek-tarif` | `cek-tarif.html` | Compare rates before creating | Header + description → card: outlet, kecamatan tujuan, berat (gram, "Setara n kg") + **Cek tarif** → idle placeholder / pending skeleton / results card ("Estimasi" badge, route + WIB time, highlight tiles Termurah · Tercepat · Pilihan, courier chips with logo + service count, service rows with logo, estimasi tiba, COD, ongkir; "Hanya cek tarif; belum bisa dipesan lewat API Mengantar" on spx/paxel/SAPLite) + rules rail (T-242) | Cek tarif | idle, pending, stale (re-check), no service, field errors, provider error + Coba lagi, route error | `checkShippingRates` |
| `/app/laporan/pengiriman` | `laporan-pengiriman.html` | Period report + CSV | Header (Ekspor CSV outline) → filter row (+ Filter lanjutan: kurir, status) → Ringkasan (T-251, spec 10 §4.6b): two panels, side by side from a 64rem summary — Volume stat strip (Total kiriman · Terkirim n + % · Retur n + % of settled · Gagal · Masih berjalan) over one composition bar that sums to the total, and Uang COD (Nilai COD · Estimasi cair with an "Estimasi" badge); no icon chips, figures not links → row (T-254, both cards the row's height): Tren harian (line COD solid / Non-COD dashed, Tabs "Jumlah kiriman" / "Nilai COD per hari", legend carrying each series' period total, data-table disclosure "Tanggal (WIB)") · Distribusi status (the four Ringkasan buckets Terkirim / Retur / Gagal / Masih berjalan, each with count · % and a share bar in its bucket colour; a bucket of more than one status opens to its statuses with badge, count, %) → Total per kurir (+ % terkirim, % retur; logo only; record list below xl) → Performa kurir → Wilayah tujuan, full width (Tabs Per provinsi / Per kota: top-10 table Wilayah · Kiriman · Terkirim · Retur · % retur with the volume bar under each name — no separate chart; "Tampilkan semua" disclosure scrolling inside a 384 px box with a pinned header; "Wilayah tidak dikenal" without a bar; record list below md) → Rute teratas, full width (outlet → kota on one line, top 5; record list below md) → Daftar kiriman table (≤ 7 columns: Nomor · Dibuat · Penerima (kecamatan, kota) · Kurir/Layanan · Status · Pembayaran · Biaya) + pagination | Ekspor CSV | empty, filtered-empty, analytics read failed (one alert; totals and list stay) | `shipment-report-repository` (`loadShipmentReportPage`, `loadShipmentReportAnalytics`), `export.csv` route |
...
| `/app/pengaturan/*` | `pengaturan.html` | Configure the gerai | Settings layout: sub-menu + content cards (Profil gerai: Identitas gerai with name read-only + WhatsApp field and one Simpan, Logo gerai, Brand gerai, prefix · Informasi label · Titik pickup list + add + internal notes · Outlet readiness · Mitra kurir switches · Koneksi Mengantar radio cards) | Per card save | invalid WhatsApp, locked prefix, refused logo (type/size/dimensions/SVG), invalid email/website/catatan, all couriers off, provider errors | `saveTenantContact`, `uploadGeraiLogo`, `removeGeraiLogo`, `saveGeraiProfile`, `saveShipmentPrefix`, `saveLabelSettings`, `saveCourierPreferences`, `savePickupPointNotes`, pickup and credential actions |
| `/app/pengaturan/label` Informasi label | Mengantar analysis §9.12 | Choose what the thermal label prints, per size (PR-86) | Settings layout → one card: size option cards (Thermal 10 × 15 / 10 × 10) → two columns: switch rows (Alamat pengirim, No. telepon pengirim, Nama penerima, No. telepon penerima, Detail alamat penerima — off prints kota and provinsi only, Peringatan sebelum retur — replaces the issue time in the footer, Logo kurir, Logo gerai, Catatan resi — T-243) · Ukuran bawaan saat mencetak (10 × 15 / 10 × 10, preselected on the label page and the batch dialog) · live preview of the real label sheet with example data (gerai name and WhatsApp as sender), scaled like the label page → footer Simpan (both sizes) | Simpan | saved / refused alert; defaults until saved; no switch for the Mengantar pickup identity (PR-71) | `saveLabelSettings`, `loadTenantLabelFields` |
...
| `/platform/tenant`, `/platform/tenant/[id]` | `platform-tenant.html` | List (search, status badges, usage) + Buat gerai; detail: KPIs, outlets, submissions, finance/reconciliation read-only, audit, prefix, **Zona berbahaya** last | `submitPlatformTenantLifecycle`, `unlockShipmentPrefix` |
...
### UX-v3.9 Invoice (nota) — PR-76–PR-80

**Object.** *Invoice* = immutable nota for one issued shipment (spec 05 DATA-14). Not a receipt: the customer pays the gerai by hand, outside GeraiCUAN.

**Flow.**
1. Entry points: detail kiriman rail (Resi terbit and later: "Cetak resi + invoice"), `/app/label/[n]`, Cetak resi list row action, and the issuance outcome "Resi terbit".
2. **Cetak resi + invoice** → server `issueShipmentInvoice(shipmentNumber)` (idempotent; returns the existing invoice) → the print view renders label then invoice → browser print → `recordLabelPrint` for the label. Two print jobs are used when label (100 mm) and invoice (80 mm / A4) media differ; the view shows them as step 1 and step 2.
3. Reprint: **Cetak invoice saja** renders the stored snapshot; no re-issue, no provider call.
4. Refusals: no resi → "Invoice terbit setelah resi terbit"; other tenant → not found.

**Invoice sheet anatomy (80 mm, top → bottom).** Gerai name (bold) · WhatsApp · address (2 lines max) → rule → `INVOICE` + number (mono) + issue time WIB → rule → Resi (mono, large) · Ekspedisi/layanan · Estimasi → Pengirim (masked label sender) · Penerima (name, kota) → Isi paket (≤ 5 lines, "+N lainnya") · Berat → rule → Ongkir · Asuransi (if > 0) · **Total ongkir** (bold, tabular) → Pembayaran: "Non-COD — dibayar di gerai" | "COD ongkir — ditagih kurir: Rp…" | "COD — ditagih kurir ke penerima: Rp…" → "Nilai barang (informasi): Rp…" → rule → footer "Nota ini bukan bukti pembayaran." (no tracking link — Cek resi needs a login, there is no public tracking page; no QR in v3.1). A4 uses the same blocks in two columns with 16 px body.

**States.** issuing (button pending), issued, reprint, not issued, error (retry keeps idempotency).

### UX-v3.10 Sign-in and sign-up — PR-83
...
| `/login/super-admin` | Same split on the dark ground (panel headline about monitoring, no "Gratis"), badge "Khusus tim GeraiCUAN", title "Masuk Admin Platform"; no sign-up link | Masuk |
...
2. **Operasional**
   - **Kiriman** → `/app/pengiriman` — both tenant roles; lifecycle queue. Create, detail, and label routes are contextual descendants and retain Kiriman as current. Tenant Admin also gets "Perbarui status dari Mengantar" here (T-204).
   - **Retur (RTS)** → `/app/pengiriman/rts` — both tenant roles; failed and returning-shipment queue. Tenant Admin also gets "Perbarui status dari Mengantar" here (T-204).
...
3. **Wawasan** (rendered as the **Laporan** group since PR-54)
   - **Laporan pengiriman** → `/app/laporan/pengiriman` — Tenant Admin only; the kiriman record per periode, outlet, kurir and lifecycle, courier performance (moved from Analitik, T-204), with CSV export (PR-55, T-165).
...
4. **Administrasi**
   - **Pengaturan** → `/app/pengaturan` — Tenant Admin only. Its own menu holds Profil gerai (`/app/pengaturan`), Informasi label (`/app/pengaturan/label`), Titik pickup (`/app/pengaturan/pickup`), Outlet (`/app/pengaturan/outlet`), Mitra kurir (`/app/pengaturan/kurir`, T-243), Koneksi Mengantar (`/app/pengaturan/koneksi`) and Anggota & akses (`/app/anggota`).
   - **Anggota & akses** → `/app/anggota` — Tenant Admin only.

**T-204 (owner decision 2026-09-25).** The product's main job is masking the Mengantar airway bill with the gerai's own label, so Impor CSV (`/app/impor`), Keuangan (`/app/keuangan`) and Analitik (`/app/analitik`) are removed from the menu and the route tree; their database tables and migrations stay. The contracts below that name those pages (UX-9, PR-42, PR-43 and mentions in other sections) are kept as history and no longer describe a live screen.
...
**Payment method (PR-64, D-12, T-186).** The draft form's "Nilai dan pembayaran" card offers three large radio choices — **Non-COD**, **COD**, **COD Ongkir** — each a bordered label with a title and a one-line consequence, ≥ 56 px tall (≥ 44 px targets below `md`), selected state by border and tint. Only the chosen method's panel is rendered, so another method's field is never in the form or submitted; its appearance is announced through a polite `role="status"` live region. Non-COD: "Nilai barang untuk asuransi (Rp)". COD: "Nilai barang (Rp)" plus a note that the total the courier collects is computed after a service is chosen (goods + ongkir + Mengantar's 3.33%) and is never typed. COD Ongkir: "Nilai barang (sudah dibayar)" (not collected), the toggle hint "(kurir menagih ongkir + biaya COD)" and the note "Nilai COD = ongkir + biaya COD 3,33%, dihitung otomatis" (D-28, T-237). The goods value typed in one panel carries to the next. The server refuses an unknown method and a zero goods value for COD and COD Ongkir.

**COD Ongkir amount (D-28, T-237).** Computed, never typed: for the selected service in "Pilih layanan ekspedisi" a muted read-only row "Nilai COD Ongkir ditagih kurir ke penerima" shows the amount in an `<output>` and the line "Dihitung otomatis: ongkir dipotong Mengantar Rp S + biaya COD 3,33% Rp F (+ pembulatan Rp R)" (`codOngkirAmount`, the version 3 break-even over the shipping Mengantar deducts). The rail lists Nilai barang (sudah dibayar), Ongkir dipotong Mengantar, Biaya COD 3,33%, Pembulatan (only when > 0) and the total "Nilai COD Ongkir"; the mobile bar shows the same total. No field is submitted: the server computes the same amount (`computedCodOngkirChargeIdr`) and refuses any other submitted value (`NOT_COMPUTED`) or a change to a recorded one (`ALREADY_RECORDED`) with an Indonesian message naming the figure. When the service has no usable shipping amount the row says so (`role="alert"`) and confirmation stays disabled. Rows recorded before D-28 with a raised charge stay valid and display as recorded. The thermal label prints "COD ONGKIR — TAGIH ONGKIR SAJA" with the charge and "Barang sudah dibayar · JANGAN DITAGIH", the goods value as "Nilai (lunas)", and no goods breakdown; the stub prints "COD ONGKIR" and the charge. Shipment detail, Laporan pengiriman and Analitik name the method.
...
- The overview has no free-floating “revenue”, “COD income”, or gross collection card. COD/non-COD cards count shipment inputs only; since T-177 (2026-09-17) they carry no declared-goods value, and they never describe receiver collection, principal liability, or revenue. Since T-204 there is no Keuangan page; the overview links no ledger workspace.
- A first-run tenant sees outlet readiness and the next setup action instead of zero-filled analytics. A genuinely healthy/empty operational day says why no work is present and links to create a shipment (T-204 removed import). Filtered empty names the active filters and offers reset.
...
- Ringkasan provides a shallow selected-versus-previous shipment trend before current work. Since T-204 the period drill-down for a Tenant Admin is Laporan pengiriman ("Laporan pengiriman" link on the summary, same range and outlet); Analitik and Keuangan are removed.

## UX-9 — Tenant analytics contract

> **Retired (T-204).** `/app/analitik` was removed; its courier comparison (item 4) lives on in Laporan pengiriman — see "T-204 — status pull and courier performance" below.
...
- Screen tenant routes as one product across Ringkasan, Kiriman, Retur (RTS), Kontak, Label, Laporan, Outlet & koneksi, and Anggota & akses. Screen platform monitoring, tenant governance, and audit as a separate platform job; never blur global and tenant scope.
...
> **Retired (T-204)** with `/app/analitik`.
...
> **Retired (T-204)** with `/app/keuangan`. The pull itself moved to Histori kiriman and Retur — see "T-204 — status pull and courier performance".
...
### T-238 — tracking history, return resi and Mengantar notes (2026-09-26)
- **Detail kiriman "Detail pelacakan"** (both roles): the courier's tracking events (`provider_order_history_events`: documented `history[{date, desc}]`, else the observed `lastHistory`), newest first, WIB, the courier's text as the title; the Tenant Admin additionally sees the Mengantar status observations (title = lifecycle label, detail = `lastHistory.desc`), and a history event an observation already shows is not repeated. Then Resi terbit and Kiriman dibuat.
- **Identity strip:** "Resi retur" (mono, copyable) after "Resi" once a pull has seen `cnote_no_rts`; absent otherwise (no placeholder).
- **Status card "Catatan dari Mengantar"** (Tenant Admin only; renders nothing when empty): badges from the newest pull observation — "Melewati batas waktu kirim" (`isBreach`), "Gagal antar terakhir: <code>" (`lastUndeliveredCode`), "Klaim disetujui/ditolak/menunggu" (`claimStatus`), "Tiket: <status>" (`ticketStatus`, hidden when `none`), "Kode POD: <code>" (`pod_code`). Urgent ones (breach, undelivered code, pending claim, open ticket) use the warn surface; the rest muted. Raw codes are shown as Mengantar sends them. No Histori filter (observations are admin-only; the Detail badge is the cheap surface).
- **Cek resi:** under the resi, "Resi retur <mono>" when known; the timeline adds the courier events with source "Kurir" (both roles).
- **Retur:** the return resi under the resi/shipment number (table) or in the meta line (phone records).
- Return progress: `RTS` + a return resi moves the shipment to "Retur dalam perjalanan"; nothing documented or captured reports "Retur diterima", so that state is still reached by no provider value (DATA-13 gap).
- **Dibatalkan (`CANCELLED`, owner 2026-09-26).** Set only when Mengantar reports `CANCELED`/`CANCELLED` (pull or webhook) on an ISSUED, IN_TRANSIT, PROBLEM or (T-247, D-33) AWAITING_UPSTREAM_PAYMENT shipment — a cancelled unpaid order is never offered "Bayar" (unpaid recovery refuses it); terminal; GeraiCUAN has no cancel action (D-30). Badge neutral + `Ban`. Histori: status-select option and a tile (tiles now Semua kiriman · Resi terbit · Dalam perjalanan · Terkirim · Dibatalkan · Perlu perhatian; "Siap dilanjutkan" stays a select option and Dasbor link only); never in Perlu perhatian. Detail rail: "Kiriman dibatalkan Mengantar. Label tidak dapat dicetak." Cetak resi: fourth tile "Dibatalkan" (`?cetak=batal`, LBL-CANCELLED) listing issued resi now cancelled, no selection checkbox, row action "Detail"; they never appear under Semua/Belum/Sudah. Label page: destructive alert "Kiriman dibatalkan — label tidak dapat dicetak" with links to detail and invoice; `recordLabelPrint` records BLOCKED `CANCELLED` (batch print skips it, "— kiriman dibatalkan"). Invoice: an issued one stays readable with a "Dibatalkan" note (screen only, not printed); a new one is refused ("Mengantar melaporkan kiriman ini dibatalkan, jadi invoice tidak diterbitkan.", its own `CANCELLED` result, T-247). Laporan/Dasbor count it in "Gagal" (spec 19).

### T-245 — Destination-area picker: local suggestions, one Mengantar lookup (PR-92, D-32, 2026-09-26)
Amends UX-10 for the shared `DestinationAreaPicker` (`src/app/app/_shared/destination-area-picker.tsx`), now used by Cek tarif, Kontak baru, Kontak detail (address add/edit) and Buat kiriman (which dropped its own `SearchPicker` instance).
- **Typing.** From 3 characters (letters or kode pos digits), suggestions come from the local wilayah reference after a 150 ms pause; no Mengantar call. Rows are grouped "Kecamatan" and "Kelurahan / desa", the group holding the best match first. Each row: the name, then "Kec. X, Kab./Kota Y, Provinsi" (a kecamatan row omits "Kec."), and the kode pos on the right when known — so Kab. Bekasi and Kota Bekasi, or the two "Sorong" kecamatan, read as different places. A 5-digit kode pos lists its kelurahan first. Placeholder "Contoh: Coblong, Dago atau 40135"; trigger text "Cari kecamatan, kelurahan atau kode pos".
- **Picking a suggestion** shows "Mencocokkan {nama} dengan Mengantar…" (status), then: exactly one strict Mengantar match → selected and the popover closes, the trigger showing the **Mengantar** label that will be stored; several (or a kecamatan pick) → "Pilih area Mengantar untuk {nama}, {Kab./Kota}" with the Mengantar options; none → the provider search opens prefilled with a notice "{nama} belum ditemukan di Mengantar. Cari langsung:"; a refusal (rate limit, busy, unavailable) → inline alert with "Coba lagi", "Cari langsung di Mengantar" and "Kembali ke saran wilayah". Typing again returns to suggestions.
- **Fallback.** Every suggestion list ends with "Cari langsung di Mengantar" (the pre-T-245 provider search, unchanged); its list ends with "Kembali ke saran wilayah". An empty local result says "Wilayah tidak ditemukan. Periksa ejaan, atau:" above that action.
- **Unchanged.** Only a Mengantar option is ever selected; hidden fields `areaId`, `areaLabel`, `areaQuery` (the provider keyword that produced the option), `areaOutletId`, `areaSelectionChanged`; the save re-validates; "Hapus pilihan"; outlet select when several outlets; keyboard combobox (cmdk: input `role=combobox`, list `role=listbox`, arrows + Enter, Escape closes, focus returns to the trigger); visible label (with the required mark in Buat kiriman); inline field error; 40 px controls on desktop and 44 px items/trigger on touch; nothing below 13 px; no horizontal overflow at 390 px.
- **Outlet first (T-247, review L11).** With several outlets and none chosen (the picker's own select, or Cek tarif's outlet field left empty), the disabled combobox reads "Pilih outlet dulu"; no suggestion lookup or Mengantar search is sent with an empty outlet.
- **Busy.** The typeahead no longer freezes after a `busy` answer: searches are single-flight and only the durable rate limit switches it to the explicit "Coba lagi" (spec 16 RATE-1).

### T-244 — Info terbaru (PR-91, D-31, 2026-09-26)
- **Menu.** "Info terbaru" (`Megaphone`) is the first item of the unlabelled top group, above Dasbor, for both roles. While the member has unread published announcements, a primary pill badge with the count (99+ above 99) sits at the row's right; the link's accessible name adds ", N belum dibaca"; on the icon rail (768–1023 px) a dot replaces the pill. Login still lands on Dasbor (`/app`); nothing remembers the last page.
- **`/app/info`** (both roles, pending gerai included). Header "Info terbaru" with "N info baru untuk Anda" (or a one-line purpose). One card per published announcement, pinned first then newest: category badge (Fitur baru · Info kurir · Jadwal · Pemeliharaan · Lainnya), "Disematkan" badge, "Baru" badge for items unread when the page opened, date "26 Sep 2026, 09.00 WIB", title (16/600), body as plain text with its line breaks (`whitespace-pre-line`; never HTML or markdown). A body over 280 characters or 4 lines is clamped to 4 lines with a "Baca selengkapnya" / "Tutup" link button (`aria-expanded`, `aria-controls`). Empty state "Belum ada info". Viewing the page marks the shown items read for this user only (idempotent); the sidebar badge clears, the "Baru" badges stay until the next visit.
- **Dasbor.** One compact status line "N info baru · Lihat info terbaru →" under the header only while unread > 0 (also on the first-run and pending-gerai Dasbor); it never replaces a Dasbor region.
- **`/platform/info`** (Admin platform). Header "Info terbaru", "N tayang · M draf", primary "Buat info". One card per announcement: drafts first, then pinned, then newest; status badge Draf (neutral) / Tayang (success) / Disematkan (info), category, "Tayang <WIB>" or "Diubah <WIB>", title, 3-line body preview; actions "Ubah" (outline) and, when live, "Turunkan" (confirm dialog naming the title). The create/edit dialog: Judul (FREE_TEXT, ≤ 120), Kategori (shadcn Select), Isi (textarea, ≤ 2,000 with a live counter, line breaks kept), "Sematkan di atas" switch; footer Batal · Simpan draf (outline; on a live item "Turunkan & simpan draf") · Tayangkan (primary; on a live item "Simpan & tetap tayang"). Field errors inline; a refused save keeps what was typed; the result line appears beside the trigger after the dialog closes.
...
- **Detail layout.** (Superseded by T-241, T-246 and T-250 — see the UX-v3.6 row: white surface, KPI cards, then two columns from a 896px content column — Riwayat kiriman and Alamat | Data kontak (one form for identity and roles) and Zona hati-hati; one column below that in the order Data kontak · Alamat · Riwayat kiriman · Zona hati-hati.) Back link to the originating list above the header. Archived: a notice, read-only Data kontak facts, no edit or archive actions.
...
- **Detail.** T-241: `/app/kontak/<role>/<n>` where `<n>` is the per-tenant contact number (DATA-19) — no UUID, name or phone in the URL (the reference's `?name=&phone=` is exactly what is avoided). The "Kembali ke daftar <role>" link, the sidebar's current item and the archive redirect follow the role segment; a contact that no longer holds that role redirects to the one it holds. History tab and page are URL state (`riwayat=cod|non-cod`, `halaman`). The pre-T-241 `/app/kontak/<id>?dari=<role>` permanently redirects to the numbered URL.
...
- **Counts.** Current snapshots that ignore any period the page carries, read in the same tenant-scoped pass as the list, each mapped to a metric ID in `docs/spec/19`. **Shares (T-247, review L9).** The "x%" and bar on each tile are the count over the page's explicit base — Histori QUE-ALL (QUE-SHARE), Retur RTS-ALL (RTS-SHARE), Cetak resi LBL-ALL + LBL-CANCELLED (LBL-SHARE) — passed by the page, never inferred from the largest tile, so Dibatalkan can never read above 100 %. On Cetak resi the list is capped at 100 rows, so when a count exceeds what is listed the page says so rather than letting the two numbers disagree.
...
- **Presets.** Hari ini, Kemarin, Minggu ini, Bulan ini, **Bulan lalu**, 7 hari terakhir, 30 hari terakhir, Rentang khusus. The active one is a checked radio (and the selected option in the collapsed select) — programmatic, never colour alone.
  - **T-240 (owner 2026-09-26) supersedes the list above for the shipped picker** (`DateRangePicker`, shadcn Popover + Calendar): exactly **Hari ini, Kemarin, 7 hari terakhir, 30 hari terakhir, Bulan ini**, in that order, as a list of buttons with `aria-pressed`, then the range calendar (a picked range is `rentang=kustom`). All WIB calendar days: Hari ini = today; Kemarin = yesterday only; 7/30 hari terakhir = the last 7/30 days including today; Bulan ini = the 1st of the month through today. The trigger names the active preset ("7 hari terakhir"); a picked range, or a legacy preset, reads as its dates. The one summary line under the filter row states the exact dates + WIB on every page that carries the picker (Laporan pengiriman and Riwayat cetak resi gained it). `minggu-ini` and `bulan-lalu` stay valid URL ids for bookmarked links but are not listed; an unknown id still falls back to 30 hari terakhir with its message.
...
- **Where it applies.** `/app` (and, until T-204, `/app/analitik` and `/app/keuangan`), and now `/app/pengiriman`, `/app/pengiriman/rts` and `/app/label`. On the three list pages it composes with the PR-52 state panel: the rows and the panel counts answer to the same window, and the default window is the shared 30 days, so an older shipment is outside the page until the range is widened.


### T-204 — status pull and courier performance

- **Perbarui status dari Mengantar** (Histori kiriman `/app/pengiriman` and Retur `/app/pengiriman/rts`, Tenant Admin only; an Operator sees neither the control nor the outlet list). It is the same read-only pull that lived on Keuangan (`pullMengantarStatus`, `src/app/app/pengiriman/status-sync-actions.ts`): GET invoices and order status from the outlet's own Mengantar account, apply lifecycle transitions (DELIVERED, PROBLEM, RTS states) and store the same settlement evidence and observation rows. One outline button with a refresh icon, no confirmation (read-only toward the provider, append-only locally). The period is the page's own range filter, capped at 62 days; with more than one outlet an "Outlet dan akun Mengantar" select sits beside the button, with one outlet the outlet is named in the note under it. Pending text is a polite status; the result alert (`role=status` on success, `role=alert` on error) receives focus and names moved, refused and unrecognised reports. The one-minute throttle per outlet account, configuration, period-too-large and provider errors keep distinct messages. The provider-basis sentence on RTS and Dasbor names this control.
- **Performa kurir** (Laporan pengiriman, between the period summary and the rows). Spec 19 M-3 unchanged from Analitik: horizontal bars of issuance rate per courier with the denominator in each label, then a closed native disclosure "Lihat detail performa kurir (n)" holding the table (Kurir, Resi terbit, Pengajuan dijawab, Tingkat penerbitan). It follows the page's outlet, courier and lifecycle filters and its period on the resolved-outcome basis, which its description states. A courier with fewer than 10 answered submissions is marked "Volume rendah (n = …)" and ranked after the higher-volume couriers. A failed read shows its own alert while the rest of the report stays usable; an empty filter says so in one line.
...
### T-242 — Cek tarif and Cek resi after the reference, with shadcn (owner 2026-09-26)

Owner: "cek-tarif.html | cek-resi.html → kau sempurnakan pakai shadcn ya … sesuaikan dengan theme kita" and "ui ux nya … lebih menarik dan dinamis. pastikan presisi." Data logic from T-237/T-238 unchanged; tokens only.

- **Cek tarif.** Page description; weight helper "Setara n kg" (live); outcome region shows an idle dashed placeholder, a skeleton of the result while pending, "Tarif perlu dicek ulang" once the route or weight changes after a result, and "Coba lagi" (outline, resubmits the same form) inside the provider-error alert. Result card: "Estimasi" info badge (spec 10 §4.11), three tiles — Termurah (`bg-tile-ok`), Tercepat (`bg-tile-info`; smallest upper day bound of the courier's own estimate, then lower bound, then price; omitted when no estimate names days), Pilihan (`bg-tile`; services · couriers · COD count). Courier chips are shadcn `Toggle`s (logo, name, service count; selected = accent + 2px primary + check), auto-fill grid so one row holds seven chips at 1440. Rows cheapest first with Termurah/Tercepat badges; a service Mengantar documents no order for (spx, paxel, SAPLite — `mengantarOrderableService`, the same rule `buildMengantarOrderRequest` refuses with) reads "Hanya cek tarif; belum bisa dipesan lewat API Mengantar". Footer: "Menampilkan n dari m layanan" and "Tarif resmi tercatat saat resi diterbitkan." Rail "Perlu diingat" lists only rules the estimate applies (estimate vs Tarif resmi, origin = outlet pickup, cargo minimum, SPX/SAP Lite, COD fee in Buat kiriman).
- **Cek resi.** Page description; input label "Nomor kiriman atau nomor resi (AWB)" with helper (the number never enters the URL); idle placeholder and pending skeleton. Result: resi mono + `CopyValueButton` "Salin nomor resi", ShipmentStatusBadge, fact tiles Ekspedisi (logo + service) · Pembayaran (method, amount, amount label — COD Ongkir shows the charge, never goods) · Tujuan (area label only), timeline newest first with the current entry highlighted, each line "… WIB · GeraiCUAN|Kurir|Mengantar", footer "Diperbarui … WIB" + Lihat rincian kiriman. Mengantar notes stay on Detail (Tenant Admin, T-238), not here.
- **Roles.** Both pages are identical for Tenant Admin and Operator; only the no-ready-outlet empty state differs (settings link for the admin).
- **Reference items skipped.** Dimensions/volumetric weight and "Kategori isi paket" (the estimate takes weight only), "Koneksi API Mengantar H2H aktif & terverifikasi" and "Live H2H API" (unverified claims; the result is labelled Estimasi), per-row "Gunakan layanan" (Buat kiriman takes no courier preselect and SPX cannot be ordered), "Kode" column (service keys are courier keys), weight-tolerance note, cashback promo, and "terupdate dalam 5–15 menit" (not a documented Mengantar fact).

### T-243 — Profil gerai & brand, Mitra kurir, pickup notes, label preview

Adopted from the owner's `pengaturan.html` reference only where GeraiCUAN has the data; adapted to the v3.2 look with the existing settings frame, cards and `OptionCard`.

- **Logo gerai** (Profil gerai). Current logo shown twice — as uploaded and as it prints (grayscale) — with "Ganti logo", "Hapus logo" (confirmation dialog) and a link to the label preview. Accepts PNG, JPEG or WebP, ≤ 200 KB, ≤ 1000 × 1000 px; SVG and mismatched bytes are refused with a specific message. Tenant Admin only.
- **Brand gerai** (Profil gerai). Catatan resi (one line, ≤ 60, counter), Kategori usaha (Fashion, tekstil & pakaian · Kosmetik & perawatan diri · Makanan & minuman kering · Elektronik & gadget · Herbal & kesehatan · Lainnya), Email CS and Website (optional, https only). WhatsApp stays in Identitas gerai (T-233).
- **Informasi label.** Three more switches — Logo kurir, Logo gerai, Catatan resi — and the gerai's default size. The logo and catatan switches say where to add the value when it is missing. The preview is the real sheet in `LabelPreviewFrame` (spec 10 §10): "Data contoh" badge, exact-size caption, zoom Pas layar / 100% / 150%, sticky beside the switches from `lg`, below them with a "Lihat pratinjau" jump link on phones. The label page and batch view share the frame.
- **Mitra kurir** (`/app/pengaturan/kurir`). One row per Mengantar courier (colour logo, name, "Ditawarkan"/"Tidak ditawarkan", `Switch`), one Simpan; at least one stays on; Ninja is never listed (D-29). Switched-off couriers disappear from Cek tarif and Buat kiriman; issued shipments are unchanged. T-247 (review L1): the server enforces it too — confirming a switched-off courier's service (even a forged post of its estimate service id) is refused inside the issuance transaction, before any batch or Mengantar call, with the inline alert "Kurir <nama> sedang dinonaktifkan di Pengaturan › Mitra kurir. Pilih layanan kurir lain, atau aktifkan kembali kurirnya."
- **Titik pickup notes.** Each point shows a "Catatan internal, tidak dikirim ke Mengantar" box (Nama PIC, WhatsApp PIC, Jadwal penjemputan, Petunjuk untuk kurir — T-252 wording) with "Tambah/Ubah catatan" in a dialog (Tenant Admin).
- **Skipped from the reference:** quota (Outlet & Kuota — no data), printer model/driver settings (the browser print path already works better), customer notifications, WITA/WIT, API/H2H connection text.

### T-253 — Pengaturan second pass (owner 2026-09-26)
- Kategori usaha: empty shows the muted placeholder "Pilih kategori usaha" before and after hydration; a set value offers "Kosongkan kategori" (muted, below a separator); the form still posts `businessCategory=""` for empty. The select takes a full row.
- Identitas gerai: Nama gerai (read-only frame) and Nomor WhatsApp gerai side by side from `sm`; an empty or malformed number gets the server's inline message (no browser bubble).
- Mitra kurir and Informasi label: the whole row toggles its switch.
- Koneksi Mengantar: the API key save is the card footer's button.

### T-252 — Pengaturan UI/UX and copy pass (owner 2026-09-26)

Owner: "di halaman app/pengaturan -> tab dan isi page di dalam tab analisa scan dan sempurnakan ui ux nya. pakai shadcn ui dan rapikan termasuk text2 di dalamnya + dropdown jika ada". Data model, Server Action contracts, validation, permissions and the label print geometry are unchanged.

- **Menu shape** (supersedes the T-156 phone index/back link, which the code no longer had): spec 10 §4.10 — a sticky rail from `lg`, one scrolling row of 44px pills below it. H1 "Pengaturan" on all seven pages, Anggota & akses included.
- **Profil gerai.** Identitas gerai: the name with its "Hubungi admin platform…" line under it, WhatsApp help states it is the sender number when a kiriman uses Alamat gerai; save "Simpan WhatsApp". Logo gerai: previews captioned "Asli" / "Hasil cetak label". Brand gerai: every field "(opsional)", "Situs web" (not "Website"), Kategori usaha shows its value before hydration and offers "Tidak diisi" to clear it; save "Simpan brand gerai".
- **Informasi label.** The size cards have a visible legend "Ukuran yang diatur" (separate from "Ukuran bawaan saat mencetak"); save "Simpan informasi label".
- **Titik pickup.** "Hapus" on the main point while other points remain opens a dialog that explains the rule ("Jadikan titik lain sebagai utama dulu…") with only "Tutup" — the server already refuses that removal. "Area asal" fills itself ("Terisi otomatis setelah alamat dipilih").
- **Outlet.** Row fact "Titik pickup utama"; the summary omits "0 perlu dilengkapi"; link buttons at the 40/44px control size.
- **Mitra kurir.** The header badge reads "n dari m aktif"; save "Simpan pilihan kurir".
- **Koneksi Mengantar.** The two sources are the shared `OptionCard`s (native radios in a fieldset); "Digunakan" still follows the persisted source. "Simpan API key" is the card's filled primary. Action messages use the screen's terms — "API key", "koneksi bawaan GeraiCUAN", "akun Mengantar sendiri" — instead of "kredensial", "default platform", "koneksi privat".
- **Anggota & akses.** Ringkasan akses uses `KpiCard` (sentence-case labels: Total anggota, Pemilik gerai aktif, Operator aktif); the last-admin protection reads "Pemilik gerai terakhir dilindungi" (spec 10 §7 roles).
- **Operator.** Unchanged: every settings URL redirects to `/app` before any settings markup renders.
...
- **Tenant login** (`app./login`). Surface badge "Untuk gerai" on a light ground with the brand top rule. Email, password with a visible "Tampilkan / Sembunyikan" button (`aria-pressed`, `aria-controls="password"`), "Masuk", then "Lupa kata sandi?" and "Belum punya akun? Daftarkan gerai". States: wrong email or password → "Email atau kata sandi salah" (never which); too many attempts → wait a minute; **email not verified** (only after the password matched) → explanation, the subject line to look for, and "Kirim ulang email verifikasi" (rate-limited, same answer for any address); **awaiting approval** (`?notice=email-terverifikasi`, where the verification link lands) → the store awaits Super Admin approval, the owner can sign in and set up outlet, pickup and Mengantar account now; failed verification link → "Minta tautan baru" to `/verifikasi-email`; `?notice=kata-sandi-diperbarui` after recovery. Audit contract kept: first `form`, `#email`, `#password`, `.auth-submit`.
- **Super Admin login** (`bos./login`). Title "Masuk Admin Platform"; dark ground, amber "Khusus tim GeraiCUAN" badge and top rule; no sign-up, no recovery link (the description says to contact the platform operator); an unverified Super Admin is told to contact the operator.
- **Daftar** (`/daftar`). Three fieldsets — Data gerai (nama gerai, nomor WhatsApp with example), Pemilik akun (nama, email "tautan verifikasi dikirim ke alamat ini"), Kata sandi (min 8, repeat, both with toggles) — and one agreement checkbox in a 44px bordered label. Invalid submit: a focused summary "Periksa N isian berikut" linking to each field, plus the message under each field (`aria-invalid`, `aria-describedby`); passwords are never echoed. Valid submit: "Periksa email Anda" naming the address (and that an already-registered address receives sign-in instructions instead), three numbered next steps (verify within 24 h, sign in and set up, wait for approval) and "Ke halaman masuk". Rate-limited: try again in an hour.
...
- **Store awaiting approval** (tenant CMS). An amber status banner "Gerai Anda menunggu persetujuan" above every page. `/app` shows "Siapkan gerai Anda": four steps with state badges — Verifikasi email (Selesai), Hubungkan akun Mengantar, Atur titik pickup (each with its button until done), Persetujuan admin platform (Menunggu). Opening a shipment page (Buat kiriman, Cek tarif, …) or submitting a shipment action lands here with "Pengiriman belum terbuka". Koneksi Mengantar hides the GeraiCUAN default for a private-only store and asks for its own API key; Outlet and the readiness copy say the connection is required.
- **Pendaftaran** (`bos./platform/pendaftaran`, sidebar "Pendaftaran"). Oldest first, one card per store: name, registered at (WIB), verification badge, owner, email, WhatsApp, Mengantar policy. "Setujui gerai" (disabled until the owner's email is verified, with the reason) opens a confirmation dialog; "Tolak pendaftaran" opens a reason field (5–500 characters, sent to the owner). The decided card stays with its result ("Gerai disetujui" / "Pendaftaran ditolak", and whether the owner was emailed) until the next visit. Empty: "Tidak ada pendaftaran yang menunggu".
...
| Daftar | Nama gerai | BUSINESS_NAME |
| Daftar | Nomor WhatsApp gerai | PHONE |
