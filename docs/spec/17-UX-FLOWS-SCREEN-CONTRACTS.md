...
| `/app/kontak/pengirim/<n>`, `/app/kontak/penerima/<n>` | `pengirim-detail.html`, `penerima-detail.html` | Read and edit one contact (T-241) | Back link → header (name, kategori badge, Aktif/Diarsipkan, "N alamat terdaftar"; WhatsApp line and wilayah = city, province of the primary address; WhatsApp · Salin nomor · primary **Buat kiriman dari kontak ini**) → 4 KPI cards from GeraiCUAN data only (Total kiriman + % terkirim · Nilai COD · Tingkat retur · Kiriman 30 hari terakhir; spec 19 CON-SHP-*; no Mengantar score) → **T-246: the page is one white surface; only the KPI row stays as (bordered) cards, every region below is a flat section with a hairline above it (spec 10 §4.11a)** → Alamat (Utama badge, Jadikan utama, Edit/Tambah dialogs; bordered white items, two columns from `md`) → Data kontak (nama, telepon, kategori and the Peran checkboxes in one form, one "Simpan data kontak" — `updateContactAction` unchanged, it always saved identity and roles together; archived: read-only facts) → Riwayat kiriman (tabs Semua/COD/Non-COD with counts; Resi & ekspedisi with logo · Tanggal · counterpart & kota tujuan · Status · Pembayaran; 10 per page; rows link to the shipment) → Zona hati-hati, a quiet section with a destructive-outline Arsipkan behind the confirmation (Tenant Admin only) | Buat kiriman dari kontak ini (links `/app/pengiriman/baru`; no preselect yet) | archived (read-only), no shipments, wrong role → the role it holds, keeping `riwayat`, `halaman` and the `tersimpan`/`diarsipkan` notice (T-247), unknown/other-tenant number → not found | `getContactByNumber`, `contact-shipment-repository`, `updateContactAction`, `add/updateContactAddressAction`, `setPrimaryContactAddressAction`, `archiveContactAction` |
...
| `/app/laporan/pengiriman` | `laporan-pengiriman.html` | Period report + CSV | Header (Ekspor CSV outline) → filter row (+ Filter lanjutan: kurir, status) → Ringkasan: 6 KPI cards (Total kiriman · Terkirim n + % · Retur n + % of settled · Masih berjalan · Nilai COD · Estimasi cair; T-235) → row: Tren harian (line COD solid / Non-COD dashed, Tabs "Jumlah kiriman" / "Nilai COD per hari", data-table disclosure) · Distribusi status (bars with status badge, count, %) → Total per kurir (+ % terkirim, % retur; logo only; record list below xl) → Performa kurir → row: Wilayah tujuan (Tabs Per provinsi / Per kota: top-10 bar + table Wilayah · Kiriman · Terkirim · Retur · % retur, "Tampilkan semua" disclosure, "Wilayah tidak dikenal") · Rute teratas (outlet → kota, top 5) → Daftar kiriman table (≤ 7 columns: Nomor · Dibuat · Penerima (kecamatan, kota) · Kurir/Layanan · Status · Pembayaran · Biaya) + pagination | Ekspor CSV | empty, filtered-empty, analytics read failed (one alert; totals and list stay) | `shipment-report-repository` (`loadShipmentReportPage`, `loadShipmentReportAnalytics`), `export.csv` route |
...
- **Detail layout.** (Superseded by T-241 and T-246 — see the UX-v3.6 row: white surface, KPI cards, flat sections Alamat · Data kontak (one form for identity and roles) · Riwayat kiriman · Zona hati-hati.) Back link to the originating list above the header. Archived: a notice, read-only Data kontak facts, no edit or archive actions.
...
- **Titik pickup notes.** Each point shows a "Catatan internal, tidak dikirim ke Mengantar" box (PIC, WhatsApp PIC, jadwal rutin, instruksi akses driver) with "Tambah/Ubah catatan" in a dialog (Tenant Admin).
