...
const workflow = [
  ["01", "Catat kiriman", "Isi data sekali, pakai kontak tersimpan."],
  ["02", "Bandingkan layanan", "Tarif mengikuti akun kurir outlet Anda."],
  ["03", "Terbitkan resi", "Nomor resi berasal langsung dari penyedia."],
...
const capabilities = [
  ...
  { icon: BookOpenCheck, label: "COD, pembukuan, dan rekonsiliasi" },
...
export default function Home() {
  return (
    <div className="min-h-dvh bg-background text-foreground">
      <a className="sr-only focus:not-sr-only focus:fixed focus:left-3 focus:top-3 focus:z-50 focus:rounded-md focus:bg-background focus:px-3 focus:py-2 focus:text-sm focus:ring-2 focus:ring-ring" href="#utama">Lewati ke konten utama</a>
      <header className="border-b bg-background">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <nav aria-label="Masuk" className="flex items-center gap-2">
            ...
            <Button asChild className="min-h-11" size="sm" variant="outline"><Link href="/login/tenant">Masuk</Link></Button>
          </nav>
        </div>
      </header>
      ...
      <main id="utama">
        <section className="border-b">
          <div className="mx-auto grid max-w-7xl gap-12 px-4 py-16 sm:px-6 sm:py-24 lg:grid-cols-[minmax(0,1.05fr)_minmax(22rem,.7fr)] lg:items-center lg:px-8 lg:py-28">
            <div>
              <p className="mb-4 text-sm font-medium text-primary">CMS operasional outlet pengiriman</p>
              ...
              <p className="mt-6 max-w-2xl text-base leading-7 text-muted-foreground sm:text-lg">Catat kiriman, terbitkan resi penyedia, cetak label, dan pisahkan dana COD dari pendapatan—dalam satu tempat untuk tim outlet.</p>
              <div className="mt-8 flex flex-wrap gap-3">
                <Button asChild className="min-h-11" size="lg"><Link href="/daftar">Daftarkan gerai <ArrowRight aria-hidden="true" /></Link></Button>
                <Button asChild className="min-h-11" size="lg" variant="outline"><Link href="/login/tenant">Masuk</Link></Button>
              </div>
              <p className="mt-4 text-sm text-muted-foreground">Pendaftaran gerai gratis. Akun aktif setelah email terverifikasi dan gerai disetujui tim GeraiCUAN.</p>
            </div>
            ...
            <div aria-label="Contoh alur kiriman" className="relative mx-auto w-full max-w-md lg:mr-0">
              ...
              <div className="rounded-lg border bg-card p-5">
                <div className="flex items-start justify-between gap-4 border-b pb-4">
                  <div><p className="text-xs text-muted-foreground">Kiriman</p><p className="mt-1 font-mono text-lg font-semibold">GC-28F9A1C4</p></div>
                  <ShipmentStatusBadge label="Resi terbit" tone="ok" />
                </div>
                <dl className="grid grid-cols-2 gap-x-5 gap-y-4 py-5 text-sm">
                  ...
                  <div><dt className="text-xs text-muted-foreground">Layanan</dt><dd className="mt-1 font-medium">Reguler</dd></div>
        ...
        <section className="border-b bg-muted/30" id="cara-kerja">
          <div className="mx-auto grid max-w-7xl gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[.55fr_1fr] lg:px-8 lg:py-20">
            <div><p className="text-sm font-medium text-muted-foreground">Cara kerja</p><h2 className="mt-2 text-3xl font-semibold tracking-tight">Dari input kiriman sampai cetak label.</h2></div>
        ...
        <section className="border-b">
          <div className="mx-auto grid max-w-7xl gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[1fr_.7fr] lg:px-8 lg:py-20">
            <div>
              <p className="text-sm font-medium text-muted-foreground">Yang dikelola</p>
        ...
        <section className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-6 px-4 py-16 sm:px-6 md:flex-row md:items-center lg:px-8">
          <div><h2 className="text-2xl font-semibold tracking-tight">Mulai dengan GeraiCUAN</h2><p className="mt-2 text-sm text-muted-foreground">Daftarkan gerai baru, atau masuk dengan akun yang sudah ada.</p></div>
          <div className="flex flex-wrap gap-3"><Button asChild className="min-h-11"><Link href="/daftar">Daftarkan gerai</Link></Button><Button asChild className="min-h-11" variant="outline"><Link href="/login/tenant">Masuk</Link></Button></div>
        </section>
      </main>
