...
import { loadPrintBrand, loadTenantLabelFields, loadTenantProfile } from "@/db/tenant-settings-repository";
...
export default async function LabelInfoSettingsPage() {
  ...
  const { brand, fields, profile } = await withTenantContext(
    db,
    principal.userId,
    principal.tenantId,
    async (tx, context) => ({
  ...
  return <LabelInfoEditor brand={brand} geraiName={profile.name} geraiWhatsapp={profile.contactWhatsapp} initial={fields} />;
}
