...
export class BulkImportRateLimitedError extends Error {
  constructor() {
    super("Bulk import rate limit exceeded.");
  }
}

export async function enforceBulkImportRateLimit(
  ...
  const result = await tx.execute<{ count: number }>(sql`
    ...
    VALUES (${context.tenantId}, ${context.userId}, 'bulk-import', 1, ${now})
    ...
    WHERE shipment_rate_limits.last_request <= ${windowStart}
       OR shipment_rate_limits.count < ${MAX_IMPORT_ATTEMPTS}
    RETURNING count
  `);

  if (result.rows.length !== 1) {
    throw new BulkImportRateLimitedError();
  }
}
