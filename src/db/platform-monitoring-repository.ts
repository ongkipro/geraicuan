...
export async function readTrend(tx: PlatformTransaction, filters: PlatformFilters): Promise<TrendBucket[]> {
  ...
  const rows = await tx.execute<{ key:string; created:string; issued:string; failed:string; unpaid:string }>(sql`
    WITH events AS (
      ...
      UNION ALL SELECT o.created_at,0,(o.status='ISSUED')::int,0,(o.status='AWAITING_UPSTREAM_PAYMENT')::int FROM ${platformMonitoringProviderOrder} o JOIN ${platformMonitoringProviderBatch} b ON b.id=o.batch_id AND b.tenant_id=o.tenant_id WHERE o.created_at>=${start} AND o.created_at<${end} ${scopeClause("o",filters.scope)} ${outletClause("b",filters.outletId)} ${courierClause("b",filters.courier)}
