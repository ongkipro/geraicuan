...
const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center rounded-lg border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap transition-all outline-none select-none focus-visible:ring-2 focus-visible:ring-ring active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive dark:aria-invalid:border-destructive/50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        ...
        // The upstream preset rings this variant with `ring-destructive/20`.
        // Measured on `/app/kontak/<id>` it is a 2px ring at 1.41:1 against the
        // card — below WCAG 1.4.11's 3:1 — and a half-alpha ring is what the
        // design contract forbids. The variant keeps its red fill and border
        // and inherits the one full-alpha 2px `--ring` from the base class.
        destructive:
      ...
      size: {
        ...
        sm: "h-9 gap-1.5 rounded-[min(var(--radius-md),12px)] px-3 text-sm in-data-[slot=button-group]:rounded-lg has-data-[icon=inline-end]:pr-1.5 has-data-[icon=inline-start]:pl-1.5 [&_svg:not([class*='size-'])]:size-3.5",
