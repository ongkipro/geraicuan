...
## 2026-09-17 — T-184 landing page: Astro in `apps/landing`

- **Shape.** One static page for `geraicuan.com` (PR-63, D-11): hero with an HTML/SVG 10 × 15 cm thermal label from sanitized sample data (real Code 128 B of a fictional AWB), four-step "Cara kerja", label sizes, a worked COD example, six reporting capabilities, a closing CTA band. Zero client JavaScript, no tracker, no application request. Astro `7.3.3` pinned exact with its own npm lockfile; not a workspace of the root app, whose `package.json`, `pnpm-lock.yaml` and `node_modules` are unchanged (sha256 and mtime checked).
- **One configuration value.** `PUBLIC_APP_ORIGIN` (default `https://app.geraicuan.com`) builds both CTAs; anything but a bare http(s) origin fails the build (checked with `/evil` → exit 1).
- **Tokens shared, with a guard.** `landing.css` copies 19 `globals.css` `:root` values verbatim; the audit's `token-parity` check fails on drift. Inter is self-hosted at build through Astro's Fonts API (Fontsource provider; the build downloads the files, so it needs network on a cold cache). Focus ring is the CMS's `:focus-visible` 2px `--ring`.
- **Copy decisions.** Kept from `src/app/page.tsx`: the workflow, label size, COD handled correctly, tenant data isolation. Dropped: "CMS gratis" (not in the PRD), the Super Admin login link (bos. is not public, D-7), "Akun tenant dibuat melalui undangan" (superseded by PR-59 sign-up), COD-as-liability accounting language (reports now carry shipping, COD fee and disbursement, T-177). Not claimed: pricing, integrations beyond Mengantar, logos, testimonials, courier counts, scanner-verified barcodes.
- **Audit.** New `scripts/ui-audit/landing-page.mjs` reuses `probe.mjs`/`cdp.mjs` and names every check. Final run PASS at 1440/390: h1 1, contrast 0 failures (109/105 nodes), weak focus 0 (14/10 probed), overflow 0, sub-44 px targets 0, CTAs all on the configured origin, 0 scripts, 0 foreign requests; secret scan of `dist/` 0 findings with canary environment values present. A build with `PUBLIC_APP_ORIGIN=http://localhost:3127` also PASS.
- **Mutations caught by name:** h1 → `h1-count`; wrong login origin → `cta-origin`; lead text `#a3a3a3` → `contrast`; `--primary` drift → `token-parity`; 1px focus outline → `focus-ring`; 32px link height → `target-size-44`; `import.meta.env.DATABASE_URL` rendered → `no-secret`. All restored byte-equal.
- **The audit caught its own modelling error first.** The COD minimality assertion initially used the half-up rounded fee for `COD − 1`, which covers goods at 118,961 because the fee rounds down; PR-9 minimality is defined on the exact 3.33%. Fixed to `(COD − 1) × 9667 < (goods + shipping) × 10000`.
- **Open for the main session.** Astro generates `apps/landing/.astro/*.d.ts`; the root ESLint flat config lints it and fails (7 errors) after any local landing build. Root `eslint.config.mjs` needs `apps/landing/**` in `globalIgnores`. Root `tsc` is unaffected (dot-directory excluded by its globs).

## 2026-09-17 — T-180 host-based surfaces: `app.`, `bos.` and the apex

One Next.js deployment now serves two hosts (PR-58, D-7). `src/proxy.ts` routes by the `Host` header through `routeByHost` (`src/lib/host-routing.ts`); configuration stays in `src/lib/auth-config.ts` (`resolveHostRouting`, new `GERAICUAN_TENANT_ORIGIN`, `GERAICUAN_PLATFORM_ORIGIN`, `GERAICUAN_PUBLIC_ORIGIN`). Scope checks remain the authority and are now host-bound: `requireCmsScope`, `resolvePlatformAccess` and the session-create hook refuse a scope on the other surface's host. Host table, environment table and the single-origin rule: `docs/spec/18-SYSTEM-MAP.md` (T-180 section); control statement: `docs/spec/12-SECURITY-ARCHITECTURE.md` SEC-7.

**Cookies.** Better Auth's cookies were already host-only (no `Domain` without `crossSubDomainCookies`); the change pins it with a test on the real `Set-Cookie` and browser evidence that the tenant and Super Admin cookies live on separate hosts.

**Links.** Better Auth's dynamic `baseURL` (`allowedHosts` = the two CMS hosts, fixed protocol, no production fallback) builds verification and recovery links from the request's own host only when it is trusted; `advanced.trustedProxyHeaders` stays unset so `X-Forwarded-Host` never steers a link. Next's own `request.url` in the proxy is built from the listen address, not the public host, so redirects are built from configured origins rather than from the request.

**Gotchas found.** (1) `next start` does not stop when `instrumentation.register` throws: it logs "Failed to prepare server" and answers every request with 500. Production now exits with status 1 instead (`src/lib/startup-validation.ts`). (2) `register` must use the documented `if (process.env.NEXT_RUNTIME === "nodejs") { await import(...) }` shape; an early `return` still bundled `node:net` into the Edge compile and broke `next dev --webpack`. (3) A second `next dev` cannot run in the same project directory (lock in `.next/dev`), and Turbopack refuses a symlinked `node_modules` outside the root, so host-mapping evidence ran from a scratch copy with `--webpack`; Chrome resolves `*.localhost` to loopback, so no host mapping file was needed.

**Checks.** New `tests/host-routing.integration.test.ts` (44), `tests/host-auth-boundary.integration.test.ts` (11), extended `tests/auth-config.integration.test.ts` (31). Full suite on 55461 after implementation: 114 files / 1,187 tests passed; final re-run 21 failures, all from a concurrent agent's unapplied migration 0050 (`cod_shipping_only`), with the auth/host/render files re-run green (8 files / 105 tests). `tsc` clean; eslint clean on `src tests scripts`. 13 mutations, each caught by a named test (listed in TASKS.md T-180). Browser: `scripts/ui-audit/host-surfaces.mjs` 12/12 on `app.`/`bos.geraicuan.localhost:3130`; production build started on 3131 refused two invalid configurations with exit 1 and routed the valid one as specified. LAN audits `admin-programme`, `header-tools`, `shipment-numbers`, `settings-ux` all exit 0 on `http://100.127.67.86:3127`. No provider call, no commit, `.delivery/` untouched.

**Left for T-181/T-183/T-185.** T-181's pages must use the reserved tenant prefixes (or change `SURFACE_PREFIXES` in the same change). T-185 documents that the reverse proxy forwards the public host in `Host` and that a health check needs a CMS `Host` header. `src/app/page.tsx` is reachable only in single-origin mode until T-184's Astro site replaces it.


## 2026-09-17 — T-186 three payment methods: Non-COD, COD, COD Ongkir

PR-64, D-12. COD Ongkir: the goods were paid outside GeraiCUAN and the courier collects a shipping charge only, editable upward from break-even `ceil(S × 10000 / 9667)` where `S` is the shipping Mengantar deducts (special, else normal, else price). Evidence and full check list: `TASKS.md` T-186.

- **Where the charge lives.** The draft form cannot know break-even (no quote yet), so the form shows the method and its goods field; the charge is chosen with the service in "Pilih layanan dan terbitkan AWB", previewed per service in the draft estimate panel, and recorded as `shipment_cod_totals` formula version 3.
- **Database moved with the rule (DATA-11).** Migration 0050 adds `shipment_drafts.cod_shipping_only`, `shipment_cod_totals.cod_shipping_basis_idr`, version 3 CHECKs (break-even, fee, split, basis), scopes the goods-inclusive sum to versions 1–2, and re-creates the COD totals INSERT policy so a COD Ongkir draft accepts only version 3 with the service's own basis — a previous-release instance computing a goods-inclusive total is refused.
- **Ledger decision.** `COD_PRINCIPAL_COLLECTABLE` is booked at 0 for COD Ongkir (no goods collected); reconciliation source matches.
- **Gotcha.** Putting the stateful field in `shipment-draft-experience.tsx` pulled `useState` into the server graph through the issuance Server Action's import and turned every dev page into a 500; the field moved to `cod-ongkir-charge.tsx` ("use client") within minutes of the report.
- **Checks.** tsc and lint clean; migration verifier PASS through 0050 on a throwaway 55462 database; `tests/cod-ongkir.integration.test.ts` 19/19; full suite on 55462 115 files / 1207 tests passed; 22 application/migration mutations and 3 live-database mutations each caught by a named test or verifier message; `payment-methods.mjs` and `cod-draft-preview.mjs` PASS at 1440/390.
- **Owed.** 0050 on the developer database, then `thermal-label.mjs` (fails there until then) and browser evidence of the issuance-panel charge and a COD Ongkir label; T-153 verification of the provisional payload.

## 2026-09-17 — T-181, T-182, T-183 self-service sign-up, approval gate, approval queue, both logins

Delivered as one change (Phase 15 re-plan). Migration `0051_self_service_sign_up.sql`. Specs: 05 DATA-12, 06 TEN-5, 12 SEC-8, 17 (T-181–T-183 section), 18 (section and inventory: 34 pages, 5 handlers, 23 action files).

- **Order.** (1) approval gate in `withTenantContext` + D-9 credential policy, tested and mutation-checked while no principal could yet be PROVISIONING; (2) PROVISIONING principal, `requireCmsScope` redirect, store-setup opt-in (layout, dashboard, four settings pages, settings actions) and the RLS moves (outlets, pickup points, Mengantar connections and secret payloads, credential rate limits, credential audit guard, prefix functions — nothing that ships); (3) `register_tenant_self_service`, verification, recovery, resend, mail, rate limits; `review_tenant_registration`, transition trigger, audit guard, queue view and `/platform/pendaftaran`; (4) both login pages.
- **Decisions.** Rejection is `ARCHIVED` with the reason in audit metadata. Approval requires a verified owner email. A self-registered store gets one outlet named after it (there is no outlet-creation path). Better Auth's `rate_limits` is not reused (it prunes rows older than its 60 s window); `public_auth_rate_limits` stores HMAC keys only. The four Better Auth recovery/verification/sign-up endpoints answer 404; server paths call them through `auth.api`. 0051 updates no existing row, so pre-existing unverified users must verify via the login resend.
- **Gotchas.** `sendResetPassword`'s own host check does not fire for `auth.api` calls (no `request`); the action's host check is the guard (mutation M15). A Server Action that revalidates refreshes the current route, which unmounted the decided approval card before its result was read — the review action no longer revalidates (all platform pages are `force-dynamic`). React did not fire `onSubmit` for a form with a function `action`; the two review forms now share one action state. The first membership-policy mutation passed vacuously because unset `app.*` settings still refused the insert; the test now forges them.
- **Checks.** `npx tsc --noEmit` 0; `npm run lint` 0; full suite on 55461: 121 files / 1,270 tests passed; migration verifier PASS through 0051 on a fresh database (writes pre-0051 tenants/users/memberships/platform role/audit rows and proves them unchanged, exact PROVISIONING policy set, ≥30 ACTIVE-only shipping policies, runtime-role probes). 28 mutations in 29 runs (M1–M15, D1–D10 with D2 re-run after strengthening, V1–V3), each caught by a named test or verifier message; DB catalog fingerprint identical before and after. Browser: `sign-up-approval.mjs` 66/66 and `host-surfaces.mjs` 12/12 on a scratch `next dev --webpack` (3132, `*.geraicuan.localhost`, disposable `geraicuan_t181_browser`), 0 contrast/focus/overflow/target findings over 34 observations at 1440/390. Named mutations and test names are in TASKS.md T-181–T-183.
- **Owed.** 0051 on the developer database (127.0.0.1:55450), then the LAN audits `admin-programme`, `header-tools`, `shipment-numbers`, `settings-ux`, `payment-methods` on `http://100.127.67.86:3127` — `admin-programme` fails there now on the missing `tenants.mengantar_credential_policy` column. Real Resend delivery and sender-domain authentication (no key here). A terms-of-use document. Disposable database `geraicuan_t181_browser` on 55461 left in place for re-runs.
- No provider call, no real email, no commit, `.delivery/` untouched, `STATUS.md` untouched.

## 2026-09-17 — T-181 security review follow-up (H1, M1, M2, L1, L2, L3, L5)

An independent security review of the Phase 15 sign-up returned FAIL. Fixed at root cause; L4 and L6 accepted as residual risk (TASKS.md T-180 notes).

- **H1 (account takeover).** An existing unverified account is never sent a plain verification link: `/daftar` re-registration and `/verifikasi-email` send a set-password link (`requestSetPasswordLink`, `src/lib/public-auth.ts`), and Better Auth `onPasswordReset` (`src/lib/auth.ts`) sets `email_verified` when an unverified account chooses a password through it. Only a newly created account gets a plain verification link. Answers unchanged. Resend and login copy now say the link sets a password (1 hour).
- **M1.** New migration `0052_sign_up_security_hardening.sql` (not an edit of 0051: drizzle's migrator applies by journal time only and never compares hashes, so an edited 0051 would be silently skipped on the developer database that already ran it). Backfills `email_verified = true` for users with an ACTIVE membership or a platform role who have no `TENANT_SELF_REGISTERED` audit event.
- **M2.** Past the per-email limit, registration answers `submitted`, recovery and resend answer `sent`, and nothing is created or sent; only the per-client limit answers `limited`.
- **L1.** Trigger `users_email_verified_guard`: anyone other than the `users` owner or a superuser may change `email_verified` only from false to true.
- **L2.** Removed the reset callback's dead host check (`auth.api` passes no Request; each action checks the host first) and fixed the comment; the verification callback now mails tenant principals only (no Super Admin, no rejected/suspended store).
- **L3.** `platform_registration_queue` also requires `app.user_id` to be an ACTIVE `SUPER_ADMIN` (`withPlatformContext` sets `app.user_id` first).
- **L5.** Verification, reset and account-exists mail carry no name; `validateRegistration` refuses `\p{Cc}`/`\p{Cf}`, and `register_tenant_self_service` (0052, same OID) refuses the Unicode Cf set.
- **Checks.** Before the fix, on 55462 at 0051: `tests/tenant-registration.integration.test.ts` 8 failed / 15 passed (each new or changed test failed on its defect). After `drizzle-kit migrate` to 0052 on 55462: same file 23/23; a combined run of 13 sign-up/auth/host/approval files gave 175/176, the one failure being `public-auth-production`'s expected subject list, updated for the set-password mail and then 3/3; all 13 files pass again in the full suite below. `npx tsc --noEmit --incremental false` exit 0; ESLint on the 13 touched source/test/script files 0 errors and 2 unused-variable warnings in the verifier, fixed and that file re-linted clean. `scripts/verify-migration-upgrade.mjs` on a fresh scratch database `geraicuan_t181_security_migration` (55462, dropped afterwards): `passed through 0052_sign_up_security_hardening.sql` (writes three extra pre-0051 unverified users, registers a store between 0051 and 0052, asserts exactly the two entitled users become verified, and probes the trigger, the queue and the format-character refusal as the runtime role). Full suite on 55462: 122 files, 1,279 tests, 8 failed — all in `cms-shell` and `cms-shell-render`, which read `src/app/_components/cms-navigation.tsx` source text changed at 11:17 by the task that owns that file; not touched here.
- **Mutations.** Each fails its named test, restored after. Code, in a scratch copy so the dev server was not churned: C1 no `onPasswordReset`, C2 resend sends a plain verification link, C3 re-registration sends one (H1 test); C4 verification callback without the principal filter (L2); C5 reset per-email limit answers `limited`, C7 resend per-email limit still sends (M2); C6 registration per-email answers `limited` (per-email limit test); C8 validation allows Cf (L5, fails with the database refusal), C9 reset mail repeats the name (L5). Database on 55462: D1 0051 function body (L5), D2 trigger dropped (L1), D3 0051 view (L3); fingerprint of the function, view, triggers and grants identical before and after (`663b2f74…`). Verifier: V1 no backfill, V2 backfill without the self-registered exclusion, V3 trigger on another column, V4 view Super Admin check bypassed, V5 function accepts format characters — each aborts with its message.
- **Browser.** Scratch copy of the working tree (`src/` and `drizzle/` diff-identical), `next dev --webpack --port 3132` with `GERAICUAN_TENANT_ORIGIN=http://app.geraicuan.localhost:3132`, `GERAICUAN_PLATFORM_ORIGIN=http://bos.geraicuan.localhost:3132`, no Resend or Mengantar key, against a fresh disposable database `geraicuan_t181_rerun_browser` on 55462 migrated by `drizzle-kit migrate` to 0052 (53 journal rows, `users_email_verified_guard` present), seeded with one Super Admin plus the `tenant@`/`super@` demo users; Chrome CDP 9430; database dropped and server stopped afterwards. `scripts/ui-audit/sign-up-approval.mjs` **66/66 PASS** on the final run, 34 probe observations at 1440/390 with 0 contrast, focus, overflow, 24 px and 44 px target findings; the resend checks pass on the set-password copy and the server log shows the resend mail as `reset-password` "Buat kata sandi untuk memverifikasi email GeraiCUAN" with the only plain `verify-email` mail sent at registration. `scripts/ui-audit/host-surfaces.mjs` **12/12** on the same server. Screenshots and reports: `scripts/ui-audit/.output/sign-up-approval/` (34 PNG, `report.json`) and `scripts/ui-audit/.output/host-surfaces/` (4 PNG, `results.json`). **Run history, not hidden:** 4 runs — on a cold dev server (empty `.next`) runs 1 and 3 stopped at 65 passed on the last step (`navigation to …/app/pengiriman/baru did not settle`): the approved store's first real render of that page reloaded about once a second, each reload initiated by the React Refresh runtime's missing-module-factory branch in the dev `webpack.js` (`if (!originalFactory) document.location.reload()`), because the page had previously only been requested while refused (server redirect), so its client modules were not yet built. Run 2 on the warm server and run 4 on a cold server after one render of the page as an ACTIVE tenant both passed 66/66. Classified as a development-compiler artifact (that branch does not exist in a production build), not an application defect; no production build was run to confirm.
- **Owed.** Real Resend delivery. (0052 on the developer database: done in the integration entry below.)

## 2026-09-17 — T-185 deploy and domain configuration, documented

Documentation only (PR-58, PR-63, D-2, D-7, D-9, D-10, D-11). No DNS change, deploy, secret, external call or application-code edit. Evidence and mutation names: `TASKS.md` T-185.

- **Where.** `docs/spec/15-DEVOPS-CICD-MIGRATIONS.md` DEP-1–DEP-6 (services, proxy requirements, build/start refusal table, DNS/TLS, roles, migrations and backup, the environment table, Resend, Mengantar); `RELEASE.md` § Production deploy runbook (setup, backup → migrate → deploy, smoke checklist, rollback, owner decisions); `OBSERVABILITY.md` (post-deploy probes, startup-refusal and mail-failure guidance); `README.md` stale build/host sections corrected; `docs/spec/18-SYSTEM-MAP.md` T-185 section.
- **The environment table is checked, not trusted.** `tests/deploy-environment-documentation.integration.test.ts` scans the deployed code (`app`: `src/`, `next.config.ts`; `migrate`: `drizzle.config.ts`; `landing`: `apps/landing`) and fails on a missing row, a stale row, a wrong service, or an environment read it cannot resolve. 26 variables. Five mutations (new read in the landing page, row deleted, stale row, wrong service, destructured `process.env`) each failed by name; files restored byte-equal.
- **Gotchas.** (1) `next build` needs configuration, not only `next start`: page-data collection imports `src/db/client.ts` and `src/lib/auth.ts`, so the build needs `APP_DATABASE_URL`, the three `GERAICUAN_*_ORIGIN` values and the three `BETTER_AUTH_*` origin/proxy values (measured on a scratch copy: exit 1 twice, then exit 0 with placeholders). The Resend values and `BETTER_AUTH_SECRET` are not needed to build; without the secret the build prints a non-fatal default-secret error per worker. (2) The shell's `grep` wrapper silently skips files it classes as binary: `src/lib/public-auth-rate-limit.ts` was missing from the first enumeration of `process.env` reads; `/usr/bin/grep -a` and the test's UTF-8 read both see it. (3) The shipment tables force row-level security, so the in-flight COD query must run as the superuser, not the owning migration role.
- **Checks.** Test file 3/3; with `auth-config` and `host-routing` on 55462: 81 passed. `npx tsc --noEmit` 0; `npm run lint` 0; `git diff --check` 0.
- **Open for the owner.** A production Super Admin bootstrap (none exists), build pack and health check, where migrations run, sending domain, platform-default Mengantar credentials, CDN, maintenance window. `.github/workflows/ci.yml` still lacks the `GERAICUAN_*_ORIGIN` build values and names `cuan.example.test`, so its `pnpm build` step cannot pass since T-180 (inferred from the measured build; not run in CI).
- `.delivery/` and `STATUS.md` untouched; no commit.

## 2026-09-17 — T-187 sidebar tree, whole group row toggles

- `cms-navigation.tsx`: each group row is one `CollapsibleTrigger` → `SidebarMenuButton` button (icon, label, chevron), so a click on the label, Enter or Space toggles; the split chevron-only button and its "Buka/Tutup grup" `aria-label` are gone. Items draw a tree with per-item `::before`/`::after` (last guide stops at its connector). All groups default open; storage key `geraicuan.cms-nav-groups.v2`; current-route group forced open; rail flyout unchanged.
- Gotcha: `SidebarMenuSubButton`'s `data-[size=md]:text-sm` outranks a plain `text-[…]` class — override with `data-[size=md]:text-[…]`.
- Gotcha: on the shared audit Chrome, `Input.dispatchMouseEvent` in the tab that ran the login flow reached no listener; continue in a fresh tab after login.
- Evidence: `scripts/ui-audit/sidebar-tree.mjs` exit 0, two mutations each caught by vitest and browser; tsc/eslint exit 0; vitest 74 passed; header-tools, admin-programme, admin-patterns pass. Details in TASKS.md T-187.

## 2026-09-17 — Phase 15 integration: dev DB 0052, CI build origins, header audit count

- **Developer database (55450).** Backed up to `~/.local/state/geraicuan-backups/geraicuan_test-55450-pre-0052-20260917T042825Z.dump`, then 0052 applied in one transaction with its Drizzle hash row: 53 migrations, trigger `users_email_verified_guard` present, 8/8 users verified.
- **CI.** `.github/workflows/ci.yml` still used `cuan.example.test` and set no `GERAICUAN_*_ORIGIN`, so its production `pnpm build` could not resolve host routing (DEP-1). The "Lint and build" step now sets tenant `https://app.example.test`, platform `https://bos.example.test`, public `https://example.test`, and trusted origins equal to tenant+platform. Checked with `next build` (Turbopack) on a scratch copy with a real `node_modules` copy and exactly the CI build variables: exit 0, 31 pages, `ƒ Proxy (Middleware)`. (A symlinked `node_modules` makes Turbopack panic with "Symlink [project]/node_modules is invalid" — a scratch-copy artifact, not a code defect.) CI itself not run.
- **`scripts/ui-audit/header-tools.mjs`.** Super Admin palette count 3 → 4 (`/platform/pendaftaran` added by T-183); re-run on `http://100.127.67.86:3127`: exit 0, 12 observations. The other LAN audits on the migrated dev DB passed: admin-programme, shipment-numbers, settings-ux, payment-methods, thermal-label.
- **Integration checks after T-180 hardening + T-187.** `npx tsc --noEmit --incremental false` 0; `npm run lint` 0; `drizzle-kit migrate` on 55461 to 0052 then `vitest run --config vitest.integration.config.mts`: 122 files, 1,280/1,280 passed.

## 2026-09-17 — T-191 landing page: three payment methods, claims re-verified

- **Change.** `apps/landing/src/pages/index.astro` adds `#pembayaran` (Non-COD, COD, COD Ongkir cards) and a COD Ongkir example table; `apps/landing/src/lib/sample.mjs` copies T-186's break-even `max(1, ceil(S × 10000 / 9667))` and difference `C − S − round_half_up(3.33% × C)`; `landing.css` adds `.methods`/`.method` on existing tokens only.
- **Stale copy corrected:** date presets, status-panel names, "biaya kirim" wording, onboarding order (own Mengantar API key, Super Admin approval), per-method label wording, sample label COD fee + VAT rows, courier list. Non-COD makes no insurance claim beyond sending the goods value (E-1).
- **Evidence.** `npm run build` complete; `landing-page.mjs` PASS at 1440/390 with new `payment-methods` and `cod-ongkir-example` checks; each failed under its mutation and passed after restore. Details: `TASKS.md` T-191.
- **Gotcha.** `astro check` is not configured for `apps/landing` (no `@astrojs/check`); the build plus the audit are the checks.

## 2026-09-17 — T-189 demo data: clean reset and a fresh local seed

- **What.** `scripts/seed-local-dev-users.mjs` rewritten: *Sekar Batik Nusantara* (id and three logins unchanged), 3 labelled pickup points, 23 contacts, 60 shipments over ~60 days across all reachable lifecycle states and all three payment methods, settlement pulls, ledger with one reversal, computed reconciliation runs, and three self-service registrations (2 PROVISIONING, 1 rejected) through the database's own registration and review functions. `pnpm db:seed-local -- --reset` deletes every non-seed tenant and user in foreign-key order and reseeds in one transaction. Evidence and removals: `TASKS.md` T-189.
- **Commands** (disposable DB on 55463; passwords only from the env file): `docker exec geraicuan-t-55463 psql -U postgres -c "CREATE DATABASE geraicuan_t189_seed"`; `npx drizzle-kit migrate`; `pnpm db:seed-local` ×2 → identical row digests; old seed + synthetic Rate Limit/T24 leftovers → `pnpm db:seed-local` ×2 (leftovers kept, identical) → `pnpm db:seed-local -- --reset` (8 tenants/10 users → 4 tenants/6 users) → plain and second reset identical; runtime-role RLS probe; ad-hoc vitest comparing seeded COD rows with `calculateCodAmounts`/`calculateCodOngkirAmounts` (26 v2, 9 v3, pass); `npx tsc --noEmit --incremental false` 0; `npx eslint` on touched files 0; `DROP DATABASE geraicuan_t189_seed`.
- **Gotchas.** (1) `shipments_allocate_public_reference` fires `BEFORE INSERT`, so `INSERT … ON CONFLICT DO UPDATE` burns a shipment number on every re-seed; the seed deletes and re-inserts its own rows and resets `tenant_shipment_counters.last_number` to the remaining maximum instead. (2) The first allocation locks an unsaved prefix; the seed restores the previous `shipment_prefix_locked_at` — read as `::text`, because a JavaScript `Date` drops the microseconds and the restored value no longer equals the original. (3) `pg` refuses a bind with more values than the statement's placeholders ("bind message supplies 5 parameters…"), so each delete passes exactly its own parameters. (4) `node` imports `src/lib/*.ts` only with type stripping (Node 22.18+); the script runs with `--disable-warning=MODULE_TYPELESS_PACKAGE_JSON`.
- **Owed.** Apply `pnpm db:seed-local -- --reset` to the developer database after its backup and after T-188; then re-run `shipment-numbers.mjs` and T-188's `t188-contact-menus.mjs` (expectations listed in T-189).

## 2026-09-17 — T-190 COD Ongkir named, with its charge, on every shipment surface

- **Change.** `presentShipmentPayment` / `PAYMENT_AMOUNT_LABELS` (`src/lib/payment-method.ts`) and `PaymentStack` (`src/components/cms/shipment-table-cells.tsx`) now render the payment cell on Histori kiriman, RTS, dashboard COD drill-down, Cek resi, Cetak resi and the bulk-import preview; repositories derive the method from `is_cod` + `cod_shipping_only` inside their tenant-scoped queries. RTS shows the method's figure instead of the declared goods value. Shipment detail uses the shared figure label. No route, action, URL state, migration or metric formula changed; `docs/spec/19` and `docs/spec/18` updated.
- **CSV.** `COD_ONGKIR` was already accepted; no charge column added because break-even needs the selected service's deducted shipping, which an import row does not have (TASKS T-190).
- **Checks.** `npx tsc --noEmit --incremental false` 0; eslint on touched files 0; full integration suite on 55462 123 files / 1308 tests passed (after one stale mock row fixed); 12 mutations all caught; `scripts/ui-audit/cod-ongkir-surfaces.mjs` PASS at 1440/390.
- **Gotcha.** `tsc` did not flag the removed `isCod` on mocked repository rows in five test files (untyped `vi.mock` fixtures); only running the suite did.
- **Owed.** Browser evidence of a real COD Ongkir row on queue, RTS, dashboard and Cetak resi once the developer database is reseeded (T-189).

## 2026-09-17 — T-188 Pengirim and Penerima menus, contact UI polish

- `/app/kontak` split into `/app/kontak/pengirim` and `/app/kontak/penerima` (one table, role flags); bare and `?peran=` URLs redirect in `next.config.ts`; create preselects via `peran`, detail follows `dari`; sidebar resolves both with `useSearchParams`. Metrics CON-SENDER-*/CON-RECIPIENT-* incl. LISTED.
- Owner polish: segmented Aktif/Diarsipkan/Semua filter beside search, short result line, cards below md, "Juga pengirim/penerima" badge with tooltip, detail header chips/actions and Kontak/Peran/Alamat cards, archived read-only. No unarchive action exists and shipments carry no contact reference, so neither "Aktifkan" nor recent shipments were added.
- Gotcha: after restoring `next.config.ts` from a mutation, the dev server kept serving the mutated redirects until the file was touched again — re-check with `curl -I` after any config restore.
- Gotcha: a headless audit tab needs `Emulation.setFocusEmulationEnabled` or the probe measures zero focus rings; and `main header` also matches the CMS shell header — scope to the page container.
- Gotcha: the shared `EmptyState` renders an `h3`; a list whose empty state can replace the table needs its own `h2` or the outline skips a level.
- Evidence: tsc/lint exit 0; integration 123 files / 1,308 passed; five mutations killed (one equivalent survived); `t188-contact-menus.mjs` PASS with probe clean; kontak-check, sidebar-tree, header-tools, admin-patterns pass. Details in TASKS.md T-188.

## 2026-09-17 — T-189 developer DB reset, full check run, audit sweep, T-192

- **Dev DB reset (55450).** Backup first: `~/.local/state/geraicuan-backups/geraicuan_test-55450-pre-t189-reset-20260917T072634Z.dump`. `pnpm db:seed-local -- --reset` applied through a temporary superuser role, created and dropped afterwards, because the container's superuser password differs from its environment. Result: 4 tenants / 6 users / 60 shipments, 11 COD Ongkir drafts.
- **Checks (main session).** `tsc` 0; lint 0; integration suite 123 files / 1308 passed; CI-env Next build 0; landing build 0.
- **Audit sweep.** 23 audits, 18 passed. The 5 failures were stale audits, not app defects, each now bound to its real rule and re-run to PASS: `thermal-label` (COD Ongkir label counted as COD; now its own kind, required, full scan), `table-ux` (kontak lists are cards below `md` by T-188 design), `shipment-references` (draft editor `?draft=<uuid>` query is not a shipment path; mutation to a UUID detail path still caught), `indonesia-operations` (PR-41 reference regex → PR-44 `PREFIX-number`), `required-checkbox` (LAN origin missing from allowlist). Details in TASKS.md T-192.
- **Gotcha.** A reseed that adds a new state (here DRAFT/ESTIMATED rows on the dashboard, a COD Ongkir label) exposes audits whose selectors were broader than their rule; check what the selector matches before calling it a regression.
- **T-192.** Sidebar icons only on Dasbor and group headers; `tests/cms-shell` 79 passed, icon re-add mutation caught, `sidebar-tree.mjs` exit 0 with screenshots inspected at 1440 and in the 390 Sheet.

- T-192 follow-up: the super-admin "Platform" group header fell back to `FileText`; it now has `ShieldCheck` (`navigationGroupIcons`). eslint 0, tsc 0, `tests/cms-shell` + `cms-shell-render` 79 passed.

## 2026-09-17 — T-194 first Super Admin bootstrap, Dockerfiles, ops image

- **Change.** `scripts/bootstrap-super-admin.mjs` (`pnpm ops:bootstrap-super-admin`): one transaction, `LOCK TABLE platform_roles`, verified ACTIVE user + `credential` account (Better Auth `hashPassword`) + SUPER_ADMIN; refuses an existing Super Admin, a taken email, a password argument, and any role without `rolsuper OR rolbypassrls`. Root `Dockerfile` (pinned `node:22.23.2-alpine3.24`, pnpm via Corepack, standalone `app` stage as `node` with a Host-aware `HEALTHCHECK`, `ops` target for `pnpm db:migrate` and the bootstrap), `apps/landing/Dockerfile` (unprivileged nginx on 8080), CI `docker-build` job (no push). Runbook and DEP-1/3/4 updated; 4 owner decisions resolved as recommended.
- **Checks.** tsc 0; eslint (touched) 0; actionlint 0; `bootstrap-super-admin` + `deploy-environment-documentation` 9 passed on 55463; 2 hash-format mutations caught (`expected 401 to be 200`); images app 308 MB / ops 1.63 GB / landing 91 MB; runtime proof on a migrated disposable DB: healthy, both hosts route, bootstrapped admin signs in on `bos.` only, missing `RESEND_API_KEY` exits 1. Details in TASKS T-194.
- **Gotcha.** `platform_roles` forces RLS with SELECT-only policies: the migration/owner role cannot create a Super Admin and sees zero existing ones, so an owner-role "already exists" check would silently pass. The bootstrap must run as the superuser.
- **Gotcha.** `/daftar` is prerendered, so its home link bakes `GERAICUAN_PUBLIC_ORIGIN` at build: Dockerfile build args (defaults = production) must equal runtime values.
- **Gotcha.** `docker build` in the sandbox failed on `~/.docker/buildx/activity` permissions; `DOCKER_CONFIG=<scratch dir>` worked. Node's `fetch` ignores a `Host` header (requests hit host routing as `127.0.0.1` → 404); use `node:http` or curl for Host-routed checks.
- **Note.** Removed a duplicate `BOOTSTRAP_PASSWORD_FILE` row that appeared in the DEP-4 table from another writer during this task (the duplicate check failed on it).

## 2026-09-17 — T-193 one "Biaya COD", VAT inside it, retired COD amounts refused

- **What.** T-179 except "Evidence precision". "Biaya COD" = `round_half_up(COD × 333 / 10000)` on Analitik, the thermal label, issuance panel, draft preview, report, disbursement estimate and ledger; VAT shown only as "termasuk PPN" (`fee × 11 / 111`). COD issuances book `MENGANTAR_COD_FEE_COST` at that fee and no `COD_SERVICE_FEE_VAT_PAYABLE`; historical VAT rows untouched and presented as "PPN dalam biaya COD (dipotong Mengantar)", outside any payable. Reconciliation classifies each issuance by its existing entries (three eras). A version 1 COD totals row not yet attempted at Mengantar is refused at confirmation (`CodTotalsFormulaRetiredError`). Label-size nits and `formatDistrictCity` fixed; seed books the new rule. Details: `TASKS.md` T-193, D-13, DATA-4, spec 19.
- **Why no migration.** Every ledger CHECK already permits an issuance without a VAT row; keeping the type valid protects history, reversals and a previous-release instance during deploy.
- **Why refuse, not recompute.** `shipment_cod_totals` is one immutable row per shipment (SELECT/INSERT grants + unique key); no version 2 row or re-estimate can replace a version 1 row.
- **Checks.** 55462 at 0052: full suite 125 files / 1414 passed; 14 mutations each killed (`scratchpad/t193/mutate.py`); seed ×3 on disposable `geraicuan_t193_seed` (dropped) — idempotent apart from `updated_at`, 32/32 fees at Mengantar's rate, 0 VAT rows; landing build 0; final tsc exit 0 and lint exit 0 (earlier runs showed 2 errors each in files concurrent tasks were editing).
- **Gotcha.** The seeder `ALTER ROLE`s the cluster-wide `geraicuan_test_runtime` to `DEV_LOCAL_PASSWORD`; on a shared test cluster pass the runtime role's existing password or every other suite on that cluster loses its login.
- **Owed.** Owner: reseed dev DB 55450 after backup; audits `cod-draft-preview`, `thermal-label`, `payment-methods`, `analytics-disclosure`, `laporan-reports`, `provider-settlement`. T-194: DEP-3/RELEASE in-flight COD wording (now "refused", plus never-attempted queued batches).

## 2026-09-17 — T-196 per-field character classes (PR-66)

- **What.** One class module and one input wrapper; the lock cleans only inserted text and shows a 3 s polite hint; every Server Action and CSV row refuses the same classes with an Indonesian message; stored values are refused on save, never rewritten.
- **Checks.** tsc 0; lint 0; full suite on 55461 125 files / 1414 passed; 6 server mutations and 2 browser mutations killed; `field-character-classes.mjs` 160/160 at 1440 and 390; `t188-contact-menus.mjs` PASS.
- **Gotcha.** Correcting an input's value inside React `onChange` must use the element's own `value` setter. The prototype setter bypasses React's value tracker, so a second identical refused keystroke fires no `onChange` and lands. Use the prototype setter only when you dispatch a synthetic `input` event yourself (the `compositionend` path).
- **Gotcha.** Audits on the shared CDP browser break when another agent clears cookies or signs in mid-run; `field-character-classes.mjs` creates its own browser context. `navigator.clipboard` is unavailable on the LAN origin (not a secure context); a real paste works via `execCommand('copy')` with `userGesture: true`, then Ctrl+V `commands:['paste']`.

## 2026-09-17 — T-195 cold-server warm-up in the sign-up audit, rail names the current page

- **What.** `sign-up-approval.mjs` opens the approval queue (1440) and the approved store's `/app/pengiriman/baru` once and waits for the same loaded document to hold 4 s before the unchanged checks (`settleDevCompilation`, dev-only reason in its comment). The icon rail's group trigger for the current section carries `aria-describedby` → hidden "Halaman saat ini: <item>"; `sweep.mjs` asserts it at 768 from Chrome's computed accessibility tree.
- **Checks.** tsc 0; eslint 0 on the 3 files; `tests/cms-shell*` on 55461 79 passed. Cold scratch server (3132, `.next` deleted per run, disposable DB on 55460, dropped after): final script 66/66 twice (runs 5, 6); earlier runs 1–2 66/66, run 3 65/66 and run 4 aborted, both at the not-yet-warmed approval queue. Sweep on `100.127.67.86:3127`: scoped 0 findings; `aria-describedby` removed → 4 rail findings; span removed with the attribute kept → same 4; restored → 0; all routes: all 21 CMS routes pass the rail rule. Screenshots `scripts/ui-audit/.output/t195-rail/`.
- **Gotcha.** Warm-up count by CDP `Page.frameNavigated` events, not by sampling `performance.timeOrigin`: a reload between samples is invisible. The real reload loop did not recur in 6 cold runs, so the settle rule was proven only against a synthetic page reloading itself six times.
- **Gotcha.** On a cold dev server a streamed page can be read with the loading boundary's h1 beside the new page's content, and one compile request redirected a signed-in Super Admin to `session-required`. Warm the page instead of loosening the check.
- **Gotcha.** A 768 probe right after navigation can still see the server-rendered expanded sidebar; wait for `[data-slot=sidebar][data-state=collapsed]` or the rail rule passes on the wrong shape.
- **Gotcha.** `pgrep -f "<pattern>"` inside a `bash -c` whose own command line contains the pattern matches that shell; `kill -- -$pid` then kills the tool call (exit 144). Look the server up by its listening port.
- **T-196 owner decision (same day).** Sender names may carry digits: `partyNameClass` makes draft/CSV sender names and sender (or dual-role) contact names BUSINESS_NAME, which now also refuses emoji; recipient names stay PERSON_NAME; the contact form lock follows the Pengirim checkbox live. PERSON_NAME hint "Hanya huruf; tanda . ' - , boleh.". Checks: tsc 0; lint 0; 9 affected files 197 passed; role mutations killed (4 and 1 failed, restored); `field-character-classes.mjs` 174/174 at 1440 and 390; full suite on 55461 **125 files / 1421 passed** (267 s).

## T-198 — Sign-up security review follow-up (2026-09-17)
- **Changed.** Seed target guard via pg's parser with field-only connect (`scripts/local-database-target.mjs`); verification link → `/verifikasi-email/konfirmasi`, which verifies only with the sign-up password (Better Auth `GET /verify-email` disabled); migration 0053 releases a rejected never-verified owner's address (`released+<id>@registration.invalid`, SUSPENDED) so the real owner can register again; server-side BUSINESS_NAME for the store name; rejection mail without typed names; production start refuses missing/short `BETTER_AUTH_SECRET` and missing/shared `APP_DATABASE_URL`; one production warning when no trusted client IP resolves.
- **Checks.** tsc 0; lint 0; full suite on 55463 128 files / 1503 passed; migration verifier through 0053 on a scratch DB (dropped); every fix killed by a mutation and restored (details under T-198 in `TASKS.md`).
- **Gotcha.** Better Auth's `getIP` falls back to `127.0.0.1` when `NODE_ENV` was `test`/`development` at *its* module load, so a `vi.stubEnv("NODE_ENV", "production")` cannot reach the production `null` path; `tests/public-auth-production` wraps `getIP` for that case.
- **Gotcha.** Under the runtime role the audit row of a review is invisible (RLS); read it after `RESET ROLE` inside the probe transaction.
- **Owed.** Migration 0053 on the developer DB 55450 and the sign-up browser audit (not run here).

## 2026-09-17 — T-199 review follow-up: input classes, COD Ongkir announcements, retired COD preview, map sync

- **Input.** One space normaliser (`normalizeSpaceSeparators`) in the client lock and every server reader (`normalizeFieldText`: Zs → space, collapse, trim), so NBSP in a paste or CSV row is a word boundary. ADDRESS takes printable punctuation; every emoji-refusing class shares one emoji set and the client drops whole grapheme clusters; person names need a letter, addresses a letter or digit, no leading combining mark.
- **Gotcha.** A lone skin-tone modifier after a space forms one grapheme cluster with the space (`Emoji_Modifier` is Extend); dropping every cluster that holds an emoji part ate the space. Drop the whole cluster only when it *is* an emoji (starts with one, or carries VS16/keycap); otherwise drop the stray part alone.
- **Gotcha.** A CDP `Runtime.evaluate` whose expression ends in `&& document.querySelector(...)` returns an element, which is not serialisable, so a wait loop reads it as false forever; end such expressions in a boolean (`!!`).
- **COD Ongkir.** The below-break-even message is visible and described, never `role="alert"`; a polite region speaks it on blur/submit, and results speak once the charge settles. **Retired COD formula.** `shipmentCodFormulaRetired` is the single T-193 decision, read by confirmation and by the detail page before rendering.
- **Docs.** Spec 19 names `shipment_cod_totals.provider_cod_amount_idr` the canonical COD fee basis (RLS insert policy keeps the order copy equal; parity test in `shipment-issuance`). Spec 18 recounted and now bound to the filesystem by `tests/system-map-inventory.integration.test.ts`.
- **Checks.** tsc 0; lint 0; affected 11 files / 243 passed; full suite 1498 passed / 4 failed (T-198: 0053 absent on 55461; konfirmasi page registration, passing on re-run); mutations and browser audits listed under T-199 in `TASKS.md`.

## 2026-09-17 — T-197 pg queries in turn on a transaction, sweep findings, rail flyout marker

- **What.** Four app call sites sent concurrent queries to one transaction client (`readPlatformHealth` `Promise.all`, platform `MonitoringView` `Promise.allSettled`, `listContacts`, shipment-draft contact search); now awaited in turn. A vitest setup file fails any test whose pg client receives a query while another runs; its first run found a fifth, in a test. Auth page padding 24px, Super Admin recovery sentence its own paragraph, prefix-unlock note capped, platform invalid-query notice in body ink, sweep screens numbered shipment/label pages, rail flyout marks the current item. Details: `TASKS.md` T-197.
- **Checks.** tsc 0; lint 0; full suite 55462 126 files / 1425 passed; 4 overlap mutations + 1 flyout mutation killed; dev server 59 overlaps → 0 on 20 routes; sweep route 69 pairs / 0 findings twice; `sidebar-tree.mjs` exit 0.
- **Finding.** The `session-required` redirect on a cold compile did not recur in 8 cold runs with access logging (all requests carried the session cookie); unrelated to the pg overlaps, which were all inside transactions while the session read is a plain pool query.
...
## 2026-09-26 — T-240 date-range presets

- **What.** The picker lists Hari ini, Kemarin, 7 hari terakhir, 30 hari terakhir, Bulan ini (in that order), then the calendar. The trigger names the preset; the summary line under the filter row carries the exact dates (added to Laporan pengiriman and Riwayat cetak resi). The period model was already complete: every id existed in `analytics-range.ts` with WIB windows and the equal-span comparison, so it is unchanged.
- **Decision kept.** Bulan ini is compared with the equal number of days before the 1st (1–26 Sep vs 6–31 Agu), per T-163; spec 19's older "same elapsed span of the previous period" sentence, which contradicted T-163, now states the rule per preset.
- **Checks.** T-240 test file + related suites 8 files / 120 passed; DB suites on the iso DB 6 / 36 passed; lint 0; tsc clean in touched files; browser Histori/Dasbor/Laporan at 1440 and 390. Screenshots in scratchpad `t240/`.

## 2026-09-26 — T-238 Mengantar tracking completion (+ CANCELLED)

- **Sources.** Mengantar Public API docs (`api-public.mengantar.com/docs/`) read through a web fetcher, no API call: webhook headers/signing/test vector and the `GET /order` example quoted as code (D); status lists, retry/timeout prose summarized (D-s). Live keys from `tests/fixtures/mengantar-order-contract.shape.json` (L): `lastHistory`, `cnote_no_rts` (3/100, always null), `isBreach`, `lastUndeliveredCode`, `claimStatus`, `ticketStatus`, `pod_code`. **No live Mengantar call.**
- **Migration 0063** (DATA-18): `provider_order_history_events` (append-only, both roles read), `provider_order_snapshots.return_cnote_no`, observation `source`/`provider_event_at`/attention columns, `SUPERSEDED`, `shipments.status` + `CANCELLED`, `record_mengantar_webhook_event` (SECURITY DEFINER) + `provider_delivery_outcome`, `print_events_cancelled_block_insert`. Applied to the iso and dev DBs (`npx drizzle-kit migrate`; the CANCELLED/print-policy/function delta added after the first apply was applied to both with the same SQL); `drizzle-kit generate` reports no drift.
- **Webhook.** Built to the docs, closed by default (`MENGANTAR_WEBHOOK_ENABLED=1` + `MENGANTAR_WEBHOOK_SECRET`, DEP env table). Our HMAC reproduces the docs' vector `05826dec…a111`. Platform-default account only.
- **Decisions.** History readable by both roles (operational; PII already visible to both); observations and attention fields stay Tenant Admin. `RTS` + return resi → `RTS_IN_TRANSIT`; `RTS_RECEIVED` unreachable (gap). Webhook deliveries older than one recorded → `SUPERSEDED`. CANCELLED only from ISSUED/IN_TRANSIT/PROBLEM, counts as Gagal, never Perlu perhatian; Histori tile replaces "Siap dilanjutkan"; Cetak resi tile `?cetak=batal`; label refused (BLOCKED `CANCELLED`), issued invoice kept with a note, no new invoice.
- **Checks.** tsc: none in T-238 files (latest run: 2 errors in another task's `tests/gerai-brand-t243.integration.test.ts`); eslint on touched files 0; T-238 + related suites on the iso DB 31 files / 358 passed (quiet window); full suite 121 files / 1,417 passed / 42 failed — all 42 in other in-flight work (contacts 0064 `tenant_contact_counters` without RLS, label-print-panel zoom, system-map counts, shell/announcements, and truncation races from a parallel run), none in a T-238 file; `verify-migration-upgrade` through 0063 on a temporary DB (created and dropped); 2 webhook mutations killed (timestamp window one-sided, signed string without the dot). Browser (dev app, isolated contexts `t238-admin`/`t238-operator`, 1440/390, no mutations; four synthetic history rows + a temporary return resi on GC-10103, deleted/reset afterwards): Detail, Cek resi, Retur, Cetak resi, Histori tiles; 0 px overflow. Screenshots: session scratchpad `t238/`.

## 2026-09-26 — T-237 Mengantar documented contract offline (D-26..D-30, SPX/cargo, Ninja removed)

- **Sources.** Mengantar Public API docs (`api-public.mengantar.com/docs/`) read through a web fetcher; **no live Mengantar call of any kind**. DATA-13 marks each key D (quoted), D-s (fetcher summary) or L (live capture/fixture).
- **What.** `buildMengantarOrderRequest` builds the documented `POST /order` body (camelCase keys, weight in kg from exact grams, `COD` vs `goodsValue`, `cargo`); the old snake_case builder is gone and the fixture transport uses the new one. Response keeps `cnote_no` as AWB authority, `isPaid`, `batch_id` from item or envelope (must agree). Pay-unpaid parses `{success, data:<count>, cnote_no:[…]}`; both sanitized fixtures rewritten to the documented shapes. Pickup slots 09.00–18.00 WIB (starts 09–17, ≥ 90 min); stored 08:00 drafts stay readable, refused at send; no migration. COD Ongkir is computed read-only (ongkir + 3,33% gross-up, existing v3 rounding); the editable charge field is gone. D-30: COD off when `unsupportedCodCheckFirstSap: true`. Cargo tiers priced from `cargoPrice`/`cargoEstimated*`, hidden below `minimumWeightCargo`. SPX stays quotable but not orderable: the docs list no SPX `courier` value (open in T-237a).
- **Ninja removed** (owner "ninja hapus aja"): gone from `MENGANTAR_COURIERS`, the display map and `public/couriers/ninja.svg`; the quote is dropped by the estimate and from stored snapshots; a historical Ninja shipment renders the plain text "Ninja" through the `CourierLogo` fallback (no `<img>`) and groups under its own name in the recap.
- **Gotcha.** After the removal, `providerCourierFromService("Ninja")` returns `"Ninja"` while `mengantarCourierOfService("Ninja")` returns `null` — intended (a historical order keeps its own group), so the catalogue parity test now checks offered keys only plus an explicit Ninja assertion.
- **Checks.** Iso DB, under `flock`: 12 T-237 files / 195 passed (order-documented, field-parity, estimate, cod-ongkir, create-flow t211, select t234, order-batch, unpaid-recovery, location-authority, issuance, reconciliation, courier-catalogue) and 15 related files / 119 passed (courier-labels, COD formula/surfaces/totals, estimate repository/options/authority, quick-rate, draft, draft-flow t205, courier performance, shipment unpaid recovery, dashboard-v3). tsc 0 errors; eslint 0 on T-237 files. `node scripts/probe-mengantar-order-documented.mjs` without the flag/env → exit 2, "Nothing was sent" (the probe itself was not run). Browser (dev app 3127, isolated context `t237`, no submit, before the Ninja removal): Buat kiriman COD Ongkir row "Rp 25.215 — ongkir Rp 24.375 + biaya COD 3,33% Rp 840", rail shows method COD Ongkir and the 390 action bar the same Rp 25.215, 0 px overflow at 390; slot list 09.00–10.00 … 17.00–18.00 WIB for tomorrow, today's list starts ≥ 90 min ahead. An earlier full-suite run by the first agent was not quiet (another suite ran concurrently on the iso DB) and is not counted as evidence; the full suite is left to the coordinator.

## 2026-09-26 — T-241 Pengirim/Penerima list + detail, contact numbers

- **What.** Contacts get a per-tenant `contact_number` (migration `0064_contact_numbers.sql`: `tenant_contact_counters`, trigger `contacts_allocate_number`, SECURITY DEFINER `allocate_contact_number()`, backfill by `(created_at, id)`, `contacts_tenant_number_key`, `contacts_contact_number_valid`) and an optional `category` (`contacts_category_valid`, runtime UPDATE granted on the column only). Detail routes `/app/kontak/{pengirim,penerima}/[nomor]`; the old `/app/kontak/[contactId]` page now only redirects. New read module `src/db/contact-shipment-repository.ts` (counts, summary KPIs, history). Migration applied with `drizzle-kit migrate` to the iso DB and the dev DB (59 dev contacts numbered).
- **Attribution rule.** No contact id on `shipment_parties`; a shipment is the contact's when its party of the page's role carries the same national phone number in the same tenant (DATA-19, spec 19 CON-SHP-*). Drafts count.
- **Gotcha.** Under the `/app` loading boundary the legacy `permanentRedirect` is streamed: the response is HTTP 200 carrying the `NEXT_REDIRECT;…;308` digest and a meta refresh, and the browser lands on the numbered URL; `notFound()` likewise renders "Kontak tidak ditemukan" with 200. Spec 18 states this instead of a raw 308.
- **Checks.** Iso DB under `flock`: `contact-numbers-t241`, `contact-actions`, `contact-lookup-screens`, `contact-directory` 4 files / 62 passed. `verify-migration-upgrade.mjs` on temp DB `geraicuan_t241_upgrade` (created and dropped on `geraicuan-test-iso`): passed through 0066. `tsc --noEmit` 0 errors; eslint 0 on T-241 files. `system-map-inventory`, `cms-shell`, `app-foundation-render` fail only on T-244's Info terbaru pages/menu (not in spec 18 yet), no kontak row missing. Browser (dev app 3127, isolated contexts): Pengirim and Penerima lists and details (`/pengirim/3`, `/penerima/7`) at 1440 and 390, 0 px document overflow, detail links numeric only, legacy `/app/kontak/<uuid>?dari=penerima` → `/app/kontak/penerima/7`, unknown uuid / number 99999 / `abc` → not found, operator detail shows KPIs and no archive zone. Fix during the check: the kategori select spanned one narrow column on the detail Kontak card (text truncated at 1440); it now spans the row.
- **Open.** "Buat kiriman dari kontak ini" links to `/app/pengiriman/baru` without a preselect (file owned by concurrent T-237 work). The phone-match expression has no index (`// lazy:` in the repository).

## 2026-09-26 — T-243 Pengaturan brand, label logos, Mitra kurir, pickup notes

- **What.** Migration `0065_gerai_brand_settings.sql` (DATA-20): `tenant_brand_settings` + label switches `show_courier_logo`/`show_gerai_logo`/`show_label_note` + pickup notes columns. Logo upload/serve (`/app/brand/logo`), Brand gerai card, Mitra kurir page, pickup notes dialog, courier print logo + gerai logo + catatan resi on the label (inline styles, `label.css` unchanged), logo on the invoice, `LabelPreviewFrame` (desk, caption, zoom, sticky), gerai default label size. Courier filter hook: `filterTenantCourierServices` (`src/lib/gerai-settings.ts`) in `cek-tarif/actions.ts`, `pengiriman/baru/page.tsx`, `pengiriman/[shipmentId]/page.tsx`.
- **Resume note.** The first agent died during browser verification; the resumed run fixed two TS errors in the new test (provider `children` now optional), then ran every check below.
- **Checks.** Iso DB under `flock`: `gerai-brand-t243` + 10 related files, 11 files / 137 passed. `verify-migration-upgrade.mjs` on temp DB `t243_upgrade_check2` (created and dropped on `geraicuan-test-iso`): passed through 0066. `tsc --noEmit` 0 errors; eslint 0 on T-243 files. `system-map-inventory` fails 4 counts (pages 41 vs 39, action files 26 vs 24, menu 13 vs 12, route rows) from T-244's Info terbaru pages/actions not yet in spec 18 — every T-243 path is in the map. Browser (dev app 3127, context `t243-resume`): real PNG upload, preview geometry measured equal with switches on/off at both sizes, label page, invoice logo, route headers/304/401; test logo and catatan removed from the dev DB after the check (invoice INV-GC-10178 was issued for the check).
- **Open.** A demo shipment whose service equals the courier name prints both ("iDexpress iDexpress"), as before T-243.

## 2026-09-26 — T-244 Info terbaru (platform announcements)

- **What.** Migration `0066_platform_announcements.sql` (DATA-21, D-31): platform-wide `platform_announcements` and insert-only per-user `platform_announcement_reads`; runtime role reads (RLS: members see published only, platform context sees drafts) and inserts its own receipts; only SECURITY DEFINER `save_platform_announcement` / `unpublish_platform_announcement` write, each re-checking an active Super Admin and auditing in the same statement. Tenant menu "Info terbaru" above Dasbor with an unread badge, `/app/info`, a Dasbor "N info baru" line; Admin platform `/platform/info`. Login landing stays `/app`.
- **Gotchas.** The unread count runs in its own tenant transaction in the layout and the Dasbor page, so a failed count hides the badge/line and never the frame. `/app/info` keeps "Baru" on cards unread at open even after the receipt revalidates the layout (state captured on mount). On the mobile Sheet the badge exists only while the menu is open. The chrome-devtools `fill` tool sets a multi-line textarea without firing React's change event (counter stayed 0); real key input works — not an app bug. A successful unpublish unmounts the row's Turunkan control, so its result line disappears with it; the Draf badge is the feedback.
- **Checks.** Iso DB under `flock`: `announcements-t244`, `cms-shell`, `system-map-inventory`, `dashboard-v3`, `app-foundation-render`, `client-bundle-boundary`, `field-character-classes` 7 files / 217 passed. `verify-migration-upgrade.mjs` on temp DB `t244_upgrade` (created and dropped on `geraicuan-test-iso`): passed through 0066. `tsc --noEmit` 0 errors; eslint 0 on T-244 files. 0066 applied with `drizzle-kit migrate` to the iso DB and the dev DB. Browser on the dev app (isolated contexts): badge, Dasbor line, `/app/info`, `/platform/info` create draft → pin + publish → unpublish, audit rows, tenant refused on `/platform/info`, 1440 and 390 without overflow.
- **Open.** The create dialog's success line stays beside "Buat info" until the next create (it can read "disimpan sebagai draf" after the row was later published from its own editor).

## 2026-09-26 — T-242 Cek tarif and Cek resi after the reference (shadcn)

- Changed: `src/app/app/cek-tarif/{page,rate-check}.tsx`, `src/app/app/cek-resi/{page,tracking-lookup}.tsx`, `tracking-result-model.ts` (unused `trackingSummaryLine` removed), `src/lib/mengantar-couriers.ts` (`mengantarOrderableService`), `src/lib/mengantar-order.ts` (refusal reuses it), tests `lookup-screens-t242` (new) and `contact-lookup-screens` (summary-line assertions dropped), specs 17/18.
- Checks run: vitest (iso DB, flock) 8 files / 142 passed + system-map-inventory 6 passed; mutation of the not-orderable note fails `lookup-screens-t242`; `tsc --noEmit` 0; eslint 0 on touched files.
- Browser: own headless Chrome (CDP 9442) against the dev app, Tenant Admin and Operator, 1440/390/720: 0 px overflow, min text 13 px, controls ≥ 40/44 px, keyboard-only lookup and chip filter. Cek tarif results only from dev audit scenarios (no live estimate); Cek resi found state from stored GC-10178 (DB lookup only).
- Open: the Tercepat tile trusts the courier's free-text estimate; Buat kiriman still shows SPX/SAP Lite without the note until confirmation refuses them.

## 2026-09-26 — T-246 UI/UX precision audit of the 2026-09-26 pages

- Changed: `src/components/app/{status-tiles,status-badge,data-card}.tsx`; `src/app/app/pengiriman/{page,rts/page}.tsx`, `src/app/app/label/page.tsx` (tile callers); `src/app/app/pengiriman/[shipmentId]/{page,detail-parts}.tsx`; `src/app/app/kontak/{contact-detail,contact-directory-list,contact-skeletons}.tsx`, new `contact-section.tsx`, `[contactId]/contact-detail-cards.tsx`, `baru/{page,contact-create-form}.tsx`; `src/app/app/pengaturan/{kurir/courier-preferences,_components/gerai-logo-card,_components/gerai-profile-card,pickup/pickup-notes,pickup/pickup-points}.tsx`; `src/app/app/label/[shipmentId]/label-preview-frame.tsx`; `src/app/app/info/announcement-parts.tsx`; `src/app/platform/info/{page,announcement-editor}.tsx`; tests `app-foundation-render` (tile columns/wrap/icons case) and `contact-lookup-screens` (flat sections, one Data kontak submit); specs 10 and 17.
- Checks run: vitest on the iso DB under `flock` — 20 files / 292 passed (app-foundation-render, announcements-t244, contact-numbers-t241, contact-lookup-screens, contact-actions, contact-directory, gerai-brand-t243, mengantar-tracking-t238, shipment-detail-t213, label-info-settings, settings-screens-t217, state-summary-panel, label-batch-print, label-print, label-render, outlet-pickup-points, client-bundle-boundary, design-token-contrast, cms-shell, lookup-screens-t242); `tsc --noEmit` 0; eslint 0 on every touched file (full `eslint` reports 1 error in `src/lib/use-typeahead-search.ts`, a concurrent uncommitted change not touched here).
- Browser: own headless Chrome (CDP 9446) against the dev app, Tenant Admin, Operator and Admin platform, before/after at 1440 and 390, tiles also at 1024 and 1280, overflow at 720 (200 % zoom): 0 px overflow everywhere, no text under 13 px outside the thermal sheet, no visible control under 40/44 px except 16 px radio dots inside 32/44 px labels; tile counts on one line per row at 1024/1280/390; keyboard Tab shows the 3 px focus ring on tiles, switches and header actions; platform editor refused empty save → 3 inline errors, 3 `aria-invalid`. Dev data touched and reverted: the seeded draft "Pemeliharaan sistem Minggu 27 September 2026" was published (to screen the Dasbor line and sidebar badge as Operator) and unpublished back to draft; one Mitra kurir switch toggled without saving.
- Open: `DestinationAreaPicker` trigger still primary-outlined (shared with Cek tarif/Buat kiriman); Dibatalkan detail state covered only by render tests.

## 2026-09-26 — T-245 Wilayah reference for fast area search (D-32)

- **What.** (a) `location.search.timing` telemetry on every destination-area lookup (spec 16 OBS-2b) and the busy-degrade fix: the typeahead is single-flight (`createTypeaheadRunner`), `busy` no longer stops auto-search, a rejected action becomes a retryable error. (b) Migration `0067_wilayah_reference.sql` (DATA-22): `pg_trgm`, tenant-neutral `wilayah_areas`, FORCE RLS, runtime SELECT only; vendored upstream data `data/wilayah/` (cahyadsn/wilayah `0d1237a5eef926629c69d287cf2282006144f4fa`, cahyadsn/wilayah_kodepos `ba8497156c5cc9bcbfc527f7b8875d403eda2354`, MIT, SOURCE.json checksums) and `npm run wilayah:import` (validate → one transaction → row count + md5 assertion → ANALYZE). (c) `searchWilayahDestinationAreas` (no provider call, no durable counter). (d) `resolveWilayahDestinationArea` (≤ 3 guarded provider keywords, strict match) and the rewritten shared `DestinationAreaPicker`; Buat kiriman adopts it. Authority unchanged: only a Mengantar option is stored or verified (D-32).
- **Import on the dev DB (55461).** `drizzle-kit migrate` applied 0067 (the mechanism 0066 used), then `npm run wilayah:import`: 38 provinsi, 514 kab/kota, 7,285 kecamatan, 83,762 kelurahan/desa, 0 without kode pos, 91,047 rows, first load heap 24.5 MB + trigram index 14.1 MB (44.4 MB total), 6.0–8.3 s; a re-run reported 0 added / 0 changed / 0 removed. After dropping the redundant `kab` search word the search text changed once (85,056 rows updated in place, fingerprint `c402ce1ccf8ccaf4bf0abe4c71e9a3bd`), and the next run again reported 0/0/0. Also applied to the iso DB (55462).
- **Measured.** EXPLAIN (ANALYZE, BUFFERS) as the runtime role on the full dev import, median of 5 (planning + execution): `coblong` 3.4 ms, `coblong bandung` 5.5, `dago` 2.4, `dago bandung` 5.0, `40135` 2.4, `bekasi` 3.8, `kab bekasi` 7.2, `kota bekasi` 5.8, `tarumajaya` 5.8, `karanganyar` 8.7, `sorong` 4.0, `tanjung` 9.0, `401` 2.0; bare province words `jawa` 35.6 (max 39.7), `barat` 32.3, `jawa barat` 18.2; every plan a bitmap scan on `wilayah_areas_search_trgm_idx`. A lone `kab` took ~95 ms, so a query of only `kab`/`kota` is refused. Server-side action time on the dev app (timing lines, 18 browser searches): `wilayah_search` median 13 ms, max 25 ms. Before: each typed pause ran two tenant transactions, a rate-limit row write, an advisory lock and a live Mengantar request — the provider share was never measured and still is not (no live call permitted); the new lines record it when one runs.
- **Checks.** Busy degrade reproduced first: `tests/location-search-busy-t245.integration.test.ts` 4/4 failed against the extracted pre-fix scheduling, 4/4 passed after. Iso DB under `flock`: 22 files / 283 passed (list in TASKS.md T-245); `deploy-environment-documentation` + `local-database-target` 25 passed. `verify-migration-upgrade.mjs` passed through `0067_wilayah_reference.sql` on temp DB `geraicuan_t245_upgrade` (created and dropped on 55462). `tsc --noEmit` exit 0; eslint exit 0 on the touched files. Five mutations each failed a named test; a sixth survived and its code was deleted as redundant (TASKS.md). Browser: own headless Chrome (CDP 9450, own profile) on `http://100.127.67.86:3127`, dev-only `contacts-area-results` audit header so the resolve step read the fixture (`providerCalls` 0 in every timing line), Buat kiriman at 1440/390: kecamatan, kode pos, Kab. vs Kota (`sorong`, `kab bekasi`), keyboard pick → auto-selected Mengantar label + hidden fields, kecamatan pick → Mengantar option list; 0 px overflow, 13 px minimum text, 40/44 px trigger and items, no console errors.
- **Open.** `pg_trgm` on the production Coolify PostgreSQL is not confirmed (RELEASE.md step 6). Cek tarif and Kontak not re-checked in a browser (render suites pass). `docker build --target ops` with the new COPY lines not run. Pre-existing failures outside T-245: `tenant-approval-gate` (brand logo route, `/app/info`) and `tenant-isolation-posture` (`tenant_contact_counters`, `contacts.category`).

## 2026-09-26 — T-247 Review fixes (suite failures, M1, L1–L11)

- **What.** Migration `0068_cancel_unpaid_and_invoice_logo.sql`: `tenant_logo_versions` (content-addressed, append-only, member-read/admin-insert RLS, backfilled from `tenant_brand_settings`), `shipment_invoices.logo_sha256` (FK to the version), `provider_delivery_outcome` replaced with `AWAITING_UPSTREAM_PAYMENT → CANCELLED` (D-33, D-34). Code: `provider-delivery-status.ts`, `unpaid-recovery-repository.ts` (cancelled gate), `order-batch-repository.ts` (`OrderCourierDisabledError`), `shipment-invoice-repository.ts` (CANCELLED in the insert WHERE, logo snapshot), `tenant-settings-repository.ts` + `brand/logo/route.ts` (`?sha=`), `invoice-sheet.tsx`, `mengantar-settlement.ts` (newest-100 history, C1 strip), `scripts/mengantar-probe-sanitize.mjs`, `status-tiles.tsx` + three callers, `contact-directory-query.ts`, `destination-area-picker.tsx`. Three stale test expectations corrected (A1–A3; A3 was the pin, not the product).
- **Gotchas.** (1) Neither the pull nor the webhook can match an unpaid order (AWB-only matching, unpaid ⇒ `cnote_no IS NULL`), so M1's transition is pinned at the decision level and the reachability is an open item. (2) `[[:cntrl:]]` in PostgreSQL's CHECK matches C1 characters in this UTF-8/en_US database — U+0085 refused a pull. (3) `tenant_logo_versions` → `tenants` is RESTRICT: a test that deletes its tenants must delete versions first (`gerai-brand-t243` does). (4) `tests/shipment-status-copy` flags any `CANCELLED: "prose"` map, even of refusal codes — the invoice refusal copy is a switch.
- **Checks run.** Focused files on the iso DB under `flock`, all green. Mutations (each reverted, each failed its guard): M1 TS transition (3 tests), M1 recovery gate, L1 courier refusal, L2 WHERE clause, L3 snapshot column / version insert / sheet source / route `sha`, L4, L5, L7 allow-list, L9 label base / explicit total, L10, L11, A3 print-zoom reset — 16/16 killed. Full suite (iso DB): first run 125/126 files, 1 failure (`shipment-status-copy`, the refusal map above); after the fix 126 files / 1,532 passed. `verify-migration-upgrade.mjs` passed through 0068 on temp DB `t247_upgrade` (created, dropped). `tsc --noEmit` 0; `npx eslint .` 0. Dev DB (55461): `drizzle-kit migrate` applied 0068 (69 migrations). Own headless Chrome (CDP 9460, own profile) as the demo Tenant Admin: `/app`, `/app/pengiriman`, `/app/label`, `/app/invoice/10178` HTTP 200, no console errors, pre-0068 invoice without logo, tile shares ≤ 100 %.
- **Open.** M1 reachability (match unpaid orders by provider order id after T-237a captures its form); webhook ordering by send time until Mengantar documents a body event time (L6); L8 documented only.

## 2026-09-26 — T-248 Status tiles as a compact stat strip

- **Changed.**
  - `src/components/app/status-tiles.tsx`: the stat strip, the full-width composition bar with the Lainnya remainder (`compositionSegments`), active dimming and the legend. `STATUS_TILE_LAYOUT` is shared with the skeleton.
  - `src/app/app/pengiriman/_list/list-states.tsx`: the skeleton strip.
  - `src/app/app/label/loading.tsx`: 4 cells.
  - `docs/spec/10-DESIGN-SYSTEM-WHITELABEL.md` §4.6.
  - `docs/spec/19-METRICS-ANALYTICS-CONTRACT.md`: the SHARE display column and QUE-/RTS-/LBL-OTHER.
  - `tests/app-foundation-render.integration.test.ts`.
  - Callers, counts, queries and hrefs are unchanged.
- **Checks run.**
  - vitest on the iso DB under `flock`: `app-foundation-render`, `shipment-lists-render` and `mengantar-tracking-t238`, 3 files / 58 passed.
  - Four mutations, each reverted and each failing its test: the bar including the "all" tile; the "all" tile showing a share; active dimming removed; the Lainnya segment removed.
  - `tsc --noEmit` 0 and `npx eslint .` 0.
- **Browser.** Own headless Chrome (CDP 9470) against the dev app on 3127, as Tenant Admin. Strip size by width:

  | Page | 1440 | 1280 | 1024 | 720 | 390 |
  |---|---|---|---|---|---|
  | Histori | 1105×116 | 945×116 | 689×189 (3×2) | 673×189 | 358×243 (3×2) |
  | Retur | 1105×116 | 945×116 | 689×189 | 673×189 | 358×263 |
  | Cetak resi | 1105×116 | 945×116 | 689×116 | 673×116 | 358×181 (2×2) |

  - Horizontal overflow is 0 px at every width, and the smallest text is 13 px.
  - Labels stay on one line from 720 px up.
  - Histori's bar is 230 + 369 + 131 + 369 px plus 3 gaps = 1105 px, the full width, ending in Lainnya 45.
  - The sr sentence reads "Resi terbit 28 (21%), Terkirim 45 (34%, dipilih), Perlu perhatian 16 (12%), Lainnya 45 (34%)".
  - Retur and Cetak resi need no remainder.
  - With `?status=DELIVERED`, the other segments dim. Tab focus shows the 3px ring.
- **Open.**
  - Phone cells run 88–107 px where labels wrap (target ~64 px).
...
- **Changed.**
  - `src/app/app/laporan/pengiriman/`: `analytics-sections.tsx` (`SectionHelp`, `reportStatusGroups`, grouped Distribusi status, trend legend totals, in-table wilayah bars, record lists below md, route line), `report-view.tsx` (grid, Ringkasan header, help alignment), `report-logic.ts` (`reportTrendTotals`), `report-trend-chart.tsx` (id-ID count tooltip), `courier-performance-chart.tsx` (row height), `loading.tsx`; `region-bar-chart.tsx` deleted.
  - `src/components/ui/tabs.tsx`: 40/44 px triggers (only this page uses Tabs).
  - Tests: `report-pages-render`, `shipment-report`. Specs: 10 §2.3 and §4.6c, 17 report row, 18 route row, 19 RPT-SHP-LIFECYCLE-GROUP, RPT-SHP-TREND-TOTALS and the M-3 wilayah row.
  - No query or formula changed.
- **Checks run.**
  - vitest on the iso DB under `flock`: `report-pages-render`, `shipment-report`, `dashboard-outcome-parity`, `host-routing`, `metric-scope-parity`, `system-map-inventory`, `cms-shell`, `dashboard-audit-scenario`, 8 files / 162 passed.
  - Two mutations, both reverted, each failing both files (5 tests): RTS_QUEUED placed in Masih berjalan, and Non-COD summed from the COD series.
  - `tsc --noEmit` 0 and `npx eslint .` 0.
- **Browser.** Own headless Chrome (CDP 9530) against dev app 3127, as Tenant Admin, `/app/laporan/pengiriman` (30 hari).
  - Before → after at 1440: Tren 452 / Distribusi 685 → both 460 (same row); Wilayah 1246 beside Rute 718 → both full width. Page 5533 → 5397 px (1024 7185 → 6395, 720 7236 → 6687, 390 7757 → 7014).
  - 0 px horizontal overflow at 1440/1024/720/390; no text under 13 px; controls under 40 px 4 → 0.
  - The Nilai COD tab legend is Rp 15.703.490, the same as the Uang COD panel. The count tooltip at 25 Sep reads COD 7 / Non-COD 9, matching the data table.
  - `kurir=JNE&rentang=7-hari` renders Terkirim 1 and Masih berjalan 5 and 0 COD in the legend. The Jan 2025 range shows the filtered-empty card.
  - Operator: redirected to `/app`.
  - Screenshots: session scratchpad `laporan2/` (`before-*`, `after-*`, `after-expanded-*`, `after-filtered-*`, `after-empty-*`).
- **Gotcha.** Radix renders only the active tab on the server, so the Nilai COD legend cannot be asserted in `renderToStaticMarkup`; it was checked in the browser.

## 2026-09-26 — T-253 Pengaturan second pass

- **Changed.**
  - `src/components/ui/select.tsx`: `SelectContent` defaults `position="popper"`, `align="start"`, `max-w-(--radix-select-content-available-width)`; `SelectItem` `max-md:min-h-11`.
  - `src/app/app/pengaturan/_components/gerai-profile-card.tsx` (Kategori placeholder on Radix `""`, "Kosongkan kategori", full row), `gerai-identity-card.tsx` (read-only name frame, 2 columns, `noValidate`), `outlet-select.tsx` (`title`); Situs web helper now says https:// is added automatically (the server already does).
  - `kurir/courier-preferences.tsx`, `label/label-info-editor.tsx` (row-sized switch `::after`), `pickup/pickup-points.tsx` (trigger `py-1.5`, hint text-sm), `pickup/pickup-notes.tsx` (`aria-invalid`), `koneksi/connection-form.tsx` (key save in the card footer).
  - Tests: `settings-screens-t217`. Specs: 10 §4.10, 17 T-253.
- **Checks run.**
  - vitest on the iso DB under `flock`: settings-screens-t217, native-select-replacement-t234, app-foundation-render, contact-lookup-screens, dashboard-v3, gerai-brand-t243, label-info-settings, outlet-settings-actions, cms-shell, tenant-approval-gate, system-map-inventory — 11 files / 214 passed.
  - Mutation: five fixes reverted one by one in a single run → 6 tests failed; restored, green again.
  - `tsc --noEmit` 0, `npx eslint .` 0, `git diff --check` clean.
- **Browser.** Own headless Chrome (CDP 9520), dev app 3127, Tenant Admin; seven settings pages at 1440/720/390 (0 px overflow; comboboxes 40/44 px); every Select app-wide opened at 1440 and 390; Kategori empty/pick/typeahead/clear; switch-row taps; four invalid submits refused inline; three dialogs trap, close on Escape and return focus. No save, no Mengantar call. Screenshots: session scratchpad `pengaturan2/`.
- **Gotcha.** A Radix Select shows its placeholder (and sets `data-placeholder` on the trigger, SSR included) only for value `""`/undefined; a sentinel item value (T-252's "__kosong__") renders as a chosen, checked value. Keep the sentinel only as the value of a "clear" item and map it back to `""` in `onValueChange`.
- **Open.** Anggota KPI cards stack at 390; "(Anda)" wraps at 1440.

## 2026-09-26 — T-252 Pengaturan UI/UX and copy pass

- **Changed.**
  - `src/app/app/pengaturan/_components/`: `settings-nav.tsx` (scrolling pill row below 1024px, sticky rail), `settings-frame.tsx` (comment), `gerai-identity-card.tsx`, `gerai-logo-card.tsx`, `gerai-profile-card.tsx` (Select value in server HTML, "Tidak diisi"), `outlet-select.tsx` (same), `shipment-prefix-card.tsx`, `settings-logic.ts` (member note copy).
  - `src/app/app/pengaturan/`: `page.tsx`, `actions.ts` (message text only), `label/label-info-editor.tsx`, `pickup/pickup-points.tsx`, `pickup/pickup-notes.tsx`, `outlet/page.tsx`, `kurir/courier-preferences.tsx`, `koneksi/connection-form.tsx` (shared `OptionCard`).
  - `src/app/app/anggota/`: `page.tsx` (`KpiCard`, H1), `loading.tsx`, `error.tsx`, `_components/role-choice.tsx`, `_components/member-access-dialog.tsx`; `src/lib/label-fields.ts` (two descriptions).
  - Tests: `settings-screens-t217`, `native-select-replacement-t234`. Specs: 10 §4.10, 17 T-252.
- **Checks run.**
  - vitest on the iso DB under `flock`: settings-screens-t217, native-select-replacement-t234, gerai-brand-t243, label-info-settings, outlet-settings-actions, cms-shell, tenant-approval-gate, system-map-inventory, dashboard-v3, shipment-invoice, contact-lookup-screens — 11 files / 202 passed.
  - Mutation: both Selects back to bare `<SelectValue />` → 2 new tests failed; reverted, green again.
  - `tsc --noEmit` 0, `npx eslint .` 0, `git diff --check` clean.
- **Browser.** Own headless Chrome (CDP 9510) against dev app 3127; Tenant Admin and Operator; 1440 and 390 (admin also 720).
  - 0 px document overflow everywhere; settings content top at 390: 515 → 227 px (menu 348 → 60 px).
  - Operator: all seven settings URLs land on `/app`.
  - Interactions (nothing saved): Kategori usaha pick and clear; Koneksi draft switch shows the blank API key field; main pickup point Hapus → explanation + "Tutup", focus back on "Hapus"; label size switch updates the preview.
  - Screenshots: session scratchpad `pengaturan/before`, `pengaturan/after`.
- **Open.** Outlet select with 2+ outlets only render-tested (demo has one outlet); the shell sidebar hides Pengaturan below the fold at 1440 × 900.

## 2026-09-26 — T-251 Laporan pengiriman summary as two panels

- **Changed.**
  - `src/app/app/laporan/pengiriman/analytics-sections.tsx`: `ReportKpiStrip` with the Volume stat strip, the composition bar and the Uang COD panel; `reportOutcomeComposition`; help text.
  - `tests/shipment-report.integration.test.ts` and `tests/report-pages-render.integration.test.ts`.
  - Spec 10 §4.6b and §4.7, the spec 17 report row, and spec 19 RPT-SHP-FAILED and RPT-SHP-OUTCOME-COMPOSITION.
  - No query, formula or metric value changed.
- **Checks run.**
  - vitest on the iso DB under `flock`: `report-pages-render`, `shipment-report` and `dashboard-v3`, 3 files / 42 passed.
  - Two mutations, each reverted and each failing its test: the Gagal predicate without CANCELLED; the Gagal segment zeroed in the view.
  - `tsc --noEmit` 0 and `npx eslint .` 0.
- **Browser.** Own headless Chrome (CDP 9500) against the dev app on 3127, as Tenant Admin, `/app/laporan/pengiriman` (30 hari):
  - Composition: 45 + 18 + 6 + 65 = 134.
  - Summary height before → after: 1440 361 → 128 (side by side); 1280 → 213 (stacked); 1024 746 → 213; 720 714 → 213; 390 722 → 317.
  - 0 px horizontal overflow at every width, and the smallest text is 13 px.
  - The zero-shipment filter shows the filtered-empty card, unchanged.
- **Gotcha.** A container query cannot style its own container: `@5xl/summary:grid-cols-3` on the `@container/summary` element never applied, while `col-span-2` on its child did, which left the money panel half-width on a second row. The grid now sits one element inside the container.

## 2026-09-26 — T-250 Contact detail in two columns

- **Changed.**
  - `src/app/app/kontak/contact-detail.tsx`: two columns below the KPI row from a 896px content column (`@container/detail`): Riwayat kiriman and Alamat | Data kontak and Zona hati-hati (sticky side, 340px); one column below with `order` (Data kontak · Alamat · Riwayat kiriman · Zona hati-hati). History table down to four columns, date under the resi.
  - `src/app/app/kontak/contact-section.tsx` (`@container/section`), `[contactId]/contact-detail-cards.tsx` (stacked fields, full-width save, 1-vs-2 address grid), `contact-form-parts.tsx` (`RoleOptions compact`), `contact-skeletons.tsx` (two-column skeleton).
  - `tests/contact-numbers-t241.integration.test.ts`, `tests/contact-lookup-screens.integration.test.ts`.
  - Spec 17 contact detail row + "Detail layout" bullet, spec 10 §4.11a, spec 18 detail row.
  - Server actions, URL state and role visibility are unchanged.
- **Checks run.**
  - vitest on the iso DB under `flock`: `contact-lookup-screens`, `contact-numbers-t241`, `contact-actions`, `contact-directory`, `system-map-inventory`, 5 files / 70 passed.
  - Two mutations, each reverted and each failing its test: the side column's `order-1` → `order-5`; the compact role effect text made visible.
  - `tsc --noEmit` 0 and `npx eslint .` 0.
- **Browser.** Own headless Chrome (CDP 9490) against the dev app on 3127, Tenant Admin and Operator; `/app/kontak/penerima/3`, `/app/kontak/pengirim/3`, `/app/kontak/pengirim/2` at 1440/1280/1024/720/390:
  - 0 px document and table overflow at every width; no text under 13 px; controls 40 px desktop / 44 px at 720 and 390.
  - Columns: 1440 main 732 + side 340; 1280 572 + 340; 1024 one column with the full sidebar, 516 + 340 with it collapsed; 720 and 390 one column in the specified order.
  - Side column sticky at 96px after a 1200px scroll at 1440 × 900 (bottom 864px).
  - Zona hati-hati only for the Tenant Admin; one submit in Data kontak. No form submitted, no data changed.
- **Open.** No dev contact with two or more addresses (2-up grid covered only by the render test); the skeleton was not rendered in a browser.
- **Gotcha.** With the full sidebar a viewport `lg` breakpoint puts two columns into a 704px content column at 1024 (main ~316px); the container query on the page content avoids that and follows the sidebar's collapsed state.

## 2026-09-26 — T-249 Buat kiriman section progress and one-screen summary rail

- **What changed.** `src/app/app/pengiriman/baru/`: `flow-parts.tsx` (section states, marker, header status, spine, required-field presence, guard text), `summary-rail.tsx` (client; `RailColumn`, three-region `SummaryRail`, `FillChecklist`, `MobileActionBar` with progress and a "Rincian" Sheet), `shipment-create-form.tsx` (renders the stepper with step-1 progress, tracks the focused section, feeds the rail), `page.tsx`, `issuance-stage.tsx`, `saved-draft-sections.tsx` (states on saved sections; the draft-number line moved into the form column so the rail can rise beside the H1). No action, validation, estimate, issuance or destination-picker change.
- **Checks run.**
  - vitest on the iso DB under `flock`: `shipment-create-flow-t211` (5 new T-249 tests: required-field presence, states and spine, marker/status per state, empty-form render with 1/4 progress + five checklist links + guard, bounded rail regions), `native-select-replacement-t234`, `tenant-approval-gate`, `cms-shell`, `shipment-lists-render`, `app-foundation-render`, `shipment-detail-t213`: 7 files / 141 passed.
  - `tsc --noEmit` 0 and `npx eslint .` 0.
- **Browser.** Own headless Chrome (CDP 9480) on the dev app 3127, Tenant Admin, dev audit header `contacts-area-results` for the area lookup (no Mengantar call; "Simpan & cek tarif" never pressed).
  - States walked empty → sections 1–2 complete → all four complete at 1440 and 390: sections `complete,current,pending,pending,locked` → `complete,complete,current,pending,locked` → all complete + locked; top bar, rail row and bottom bar read 1/4 → 2/4 → 4/4; spine segments `done` only between complete sections. Tab from section 2 into 3 and 4 moves "current" with focus.
  - Rail footer bottom (window height): empty 1440×900 717 (900), 1366×768 717 (768), 1280×720 704 (720, body scrolls); filled COD 725 / 725 / 704, 688 at page end. Rail top 96 at scroll 0 and when sticky.
  - 390×844: no horizontal overflow (also 720); Sheet opens from "Rincian" with the full summary, Escape returns focus to "Rincian"; last card ends 20px above the bar (bar 105px, page padding 128px).
- **Gotcha.** A rail that simply grew (the first checklist design) pushed "Simpan & cek tarif" below a 900px window before scrolling, because the rail starts 160px down; rising it beside the H1 (`-mt-16`, top 96) makes the initial and sticky positions equal, so one `max-height` bounds both.
- **Open.** Estimate and issuance stages not rendered in a browser (no reachable ESTIMATED dev draft without a provider-verified save).

## 2026-09-26 — T-254 follow-up: status bucket map flagged as a second label map
- **Symptom.** `tests/shipment-status-copy` › "has no second status-to-label map" failed on all 14 statuses in `src/app/app/laporan/pengiriman/analytics-sections.tsx`; T-254's focused run did not include that suite.
